...
#ifndef EN_PROP_IOBJ_DISC_FT_H
#define EN_PROP_IOBJ_DISC_FT_H
...
class EnglishPropIObjFT : public P1DescFeatureType {
public:
	EnglishPropIObjFT() : P1DescFeatureType(Symbol(L"iobj-prop")) {}
								...
								DTFeature **resultArray) const
	{
			...
			if(prop->getPredType() == Proposition::VERB_PRED)
			{
				for(int j =0; j< prop->getNArgs(); j++)
				{
					if(prop->getArg(j)->getType() == Argument::MENTION_ARG){
						if((prop->getArg(j)->getRoleSym() == Argument::IOBJ_ROLE) && 
		...
		if (discard) {
			SessionLogger::warn("DT_feature_limit") <<"EnglishPropIObjFT discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
		}
		return n_results;
	}
};
