...
#ifndef EN_EL_SAME_TRIGGER_DIST_FEATURE_TYPE_H
#define EN_EL_SAME_TRIGGER_DIST_FEATURE_TYPE_H
...
class EnglishELSameTriggerDistFeatureType : public EventLinkFeatureType {
public:
	EnglishELSameTriggerDistFeatureType() : EventLinkFeatureType(Symbol(L"same-trigger-distance")) {}
								...
								DTFeature **resultArray) const
	{
		...
		if (stem1 == stem2) {
			resultArray[0] = _new DTQuadgramFeature(this, state.getTag(), 
									v1->getEventType(), Symbol(L"same-trigger"), distance);
			return 1;
		}
		...
		if (wcscmp(str1.substr(0,3).c_str(), str2.substr(0,3).c_str()) == 0) {
			resultArray[0] = _new DTQuadgramFeature(this, state.getTag(), 
									v1->getEventType(), Symbol(L"similar-trigger"), distance);
			return 1;
		}
