...
#ifndef EN_EL_SAME_TRIGGER_SAME_ARG_FEATURE_TYPE_H
#define EN_EL_SAME_TRIGGER_SAME_ARG_FEATURE_TYPE_H
...
class EnglishELSameTriggerSameArgFeatureType : public EventLinkFeatureType {
public:
	EnglishELSameTriggerSameArgFeatureType() : EventLinkFeatureType(Symbol(L"same-trigger-same-arg")) {}
								...
								DTFeature **resultArray) const
	{
				...
				for (int j = 0; j < v2->getNArgs(); j++) {
						...
						o->getNthArgEntity(v1, i) == o->getNthArgEntity(v2, j))
					{
						...
						resultArray[count++] = _new DTQuadgramFeature(this, state.getTag(), 
							v1->getEventType(),	Symbol(L"same-trigger"), v1->getNthArgRole(i));
					}
				}
			}
		}
			...
			for (int i = 0; i < v1->getNArgs(); i++) {
				...
				for (int j = 0; j < v2->getNArgs(); j++) {
						...
						o->getNthArgEntity(v1, i) == o->getNthArgEntity(v2, j))
					{
						...
						resultArray[count++] = _new DTQuadgramFeature(this, state.getTag(), 
							v1->getEventType(),	Symbol(L"similar-trigger"), v1->getNthArgRole(i));
					}
				}
			}
		}
		...
		if (discard) {
			SessionLogger::warn("DT_feature_limit") <<"en_ELSameTriggerSameArgFeatureType discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
		}
		return count;
	}
};
