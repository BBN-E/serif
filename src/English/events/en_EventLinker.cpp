...
EnglishEventLinker::EnglishEventLinker(): _use_correct_answers(false) {
	_use_correct_answers = ParamReader::isParamTrue("use_correct_answers");
}

void EnglishEventLinker::linkEvents(const EventMentionSet *eventMentionSet, 
							 EventSet *eventSet,
...
Event *EnglishEventLinker::findBestCandidate(EventMention *em, const EntitySet *entitySet, 
									  const PropositionSet *propSet) 
{
...
bool EnglishEventLinker::anchorIsDefiniteNounPhrase(EventMention *em)
{
	if (em->getAnchorProp() == 0)
		return false;
