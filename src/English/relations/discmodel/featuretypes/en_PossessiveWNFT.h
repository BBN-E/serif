...
#ifndef EN_POSSESSIVE_WN_FT_H
#define EN_POSSESSIVE_WN_FT_H
...
class EnglishPossessiveWNFT : public P1RelationFeatureType {
public:
	EnglishPossessiveWNFT() : P1RelationFeatureType(Symbol(L"poss-wordnet")) {}
		...
		if (o->hasPossessiveRel()) {
			...
			while (level >= 0) {
				if (nfeatures >= DTFeatureType::MAX_FEATURES_PER_EXTRACTION){
					SessionLogger::warn("DT_feature_limit") << "en_PossessiveWNFT discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
					break;
				}
	...
	static int startLevel() {
		...
		if (!init) {
			...
			if (_start_offset <= 0) {
				throw UnexpectedInputException("EnglishPossessiveWNFT::startLevel()",
	...
	static int interval() {
		...
		if (!init) {
			...
			init = true;
			if (_interval <= 0) {
				throw UnexpectedInputException("EnglishPossessiveWNFT::interval()",
