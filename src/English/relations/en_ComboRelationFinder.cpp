...
#include "English/relations/en_SpecialRelationCases.h"
...
UTF8OutputStream EnglishComboRelationFinder::_debugStream;
bool EnglishComboRelationFinder::DEBUG = false;

Symbol EnglishComboRelationFinder::NO_RELATION_SYM = Symbol(L"NO_RELATION");

EnglishComboRelationFinder::EnglishComboRelationFinder() : _allow_mention_set_changes(false), _skip_special_case_answer(false) {

	_use_correct_answers = ParamReader::isParamTrue("use_correct_answers");

	EnglishP1RelationFeatureTypes::ensureFeatureTypesInstantiated();
	...
	if (!p1_model_file.empty()) {
		...
		if (overgen_percentage == 0 && ParamReader::hasParam("p1_relation_undergen_percentage")) {
			double under = ParamReader::getOptionalFloatParamWithDefaultValue("p1_relation_undergen_percentage", 0);
...
EnglishComboRelationFinder::~EnglishComboRelationFinder() {
	...
	if (_p1Weights) {
		...
		for (iter = _p1Weights->begin(); iter != _p1Weights->end(); ++iter) {
			DTFeature* feature = (*iter).first;
			delete feature;
		}
		delete _p1Weights;
	}
	if (_p1SecondaryWeights) {
		DTFeature::FeatureWeightMap::iterator iter;
		for (iter = _p1SecondaryWeights->begin(); iter != _p1SecondaryWeights->end(); ++iter) {
			DTFeature* feature = (*iter).first;
			delete feature;
		}
	...
	if (_maxentWeights) {
		DTFeature::FeatureWeightMap::iterator iter;
		for (iter = _maxentWeights->begin(); iter != _maxentWeights->end(); ++iter) {
			DTFeature* feature = (*iter).first;
			delete feature;
		}
...
void EnglishComboRelationFinder::cleanup() {
	delete _observation;
	_observation = 0;
}

void EnglishComboRelationFinder::resetForNewSentence() {
	_n_relations = 0;
}
...
RelMentionSet *EnglishComboRelationFinder::getRelationTheory(EntitySet *entitySet,
											   ...
											   const PropTreeLinks* ptLinks)
{
	...
	if (mentionSet == 0) {
		throw InternalInconsistencyException("EnglishComboRelationFinder::getRelationTheory()",
											"MentionSet is null.");
	}
	...
	for (int i = 0; i < nmentions; i++) {
		...
		for (int j = i + 1; j < nmentions; j++) {
			...
			Symbol maxentAnswer = _maxentTagSet->getNoneTag();
	...
	for (int p = 0; p < propSet->getNPropositions(); p++) {
		...
		for (int a = 0; a < prop->getNArgs(); a++) {
			if(prop->getArg(a)->getType() != Argument::MENTION_ARG){
				//std::cout<<"en_combo_relation_finder skipping non mention arg"<<std::endl;
				continue;
			}
 ...
 *  this will do for now.  It's tuned for English ACE, and may not work 
 ...
 */
double EnglishComboRelationFinder::calculateConfidenceScore(const Mention* mention1, const Mention* mention2, 
...
Symbol EnglishComboRelationFinder::findSpecialCaseRelation(MentionSet *mentionSet) {

	RelationPropLink *propLink = _observation->getPropLink();

	// here in XXX
	if (_observation->getMention1()->getNode()->getHeadWord() == EnglishWordConstants::HERE &&
...
void EnglishComboRelationFinder::addRelation(const Mention *first, const Mention *second, Symbol type, float score) {
...
Symbol EnglishComboRelationFinder::fixEthnicityEtcRelation(const Mention *first, const Mention *second) {
...
void EnglishComboRelationFinder::findMentionSetChanges(MentionSet *mentionSet) {
	...
	if (unknownMention != 0) {
		...
		if (unknownMention->getMentionType() == Mention::PRON &&
			(headword == EnglishWordConstants::THEIR ||
			headword == EnglishWordConstants::THEY ||
			headword == EnglishWordConstants::THEM ||
			headword == EnglishWordConstants::WHO ||
			headword == EnglishWordConstants::WHOM ||
			headword == EnglishWordConstants::WHOSE))
		{
	...
	if (pronMention != 0 && pronMention->getEntityType().matchesPER()) {
		...
		if (headword == EnglishWordConstants::WE ||
...
void EnglishComboRelationFinder::tryForcingRelations(MentionSet *mentionSet) {
...
bool EnglishComboRelationFinder::tryFakeTypeRelation(MentionSet *mentionSet,
...
/*bool EnglishComboRelationFinder::tryCoercedTypeRelation(MentionSet *mentionSet, EntitySet *entitySet,
												 const Mention *possMention,
												 EntityType possibleType)
{
	EntityType origType = possMention->getEntityType();
...
bool EnglishComboRelationFinder::shouldBeReversed(Symbol answer) {
	return shouldBeReversedScore(answer) > 0;
}

float EnglishComboRelationFinder::shouldBeReversedScore(Symbol answer) {
...
void EnglishComboRelationFinder::loadSymbolHash(SymbolHash *hash, const char* file) {
	...
	if (stream.fail()) {
		...
		err.append(file);
		throw UnexpectedInputException("EnglishComboRelationFinder::loadSymbolHash()",
			(char *)err.c_str());
	}
