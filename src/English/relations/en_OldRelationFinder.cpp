...
OldRelationFinder::OldRelationFinder() {
	...
	if (parameter == "EELD")
		...
		std::string error = "Parameter 'relation_finder_type' must be set to 'ACE', 'EELD' or 'ITEA'";
...
/*if (_use_correct_answers) {
		...
		for (int k = 0; k < mentionSet->getNMentions(); k++) {
			...
			for (int m = k+1; m < mentionSet->getNMentions(); m++) {
				...
				if (s1->getParent() != 0 && s1->getParent()->getTag() == EnglishSTags::NPP)
					s1 = s1->getParent();
				const SynNode *s2 = ment2->getNode()->getHeadPreterm();
				if (s2->getParent() != 0 && s2->getParent()->getTag() == EnglishSTags::NPP)
					s2 = s2->getParent();
...
bool OldRelationFinder::isGroupMention(const Mention* mention) {
	...
	if (headword == EnglishWordConstants::THEY ||
		headword == EnglishWordConstants::THEM ||
		headword == EnglishWordConstants::WE ||
		headword == EnglishWordConstants::THESE ||
		headword == EnglishWordConstants::THOSE)
		return true;
	Symbol headtag = mention->getNode()->getHeadPreterm()->getTag();
	if (headtag == EnglishSTags::NNPS ||
		headtag == EnglishSTags::NNS)
		return true;
	return false;
}
...
bool OldRelationFinder::findWithCoercedType(Argument *leftArg, Argument *rightArg, Proposition *prop)
{
	...
	if (newType.isRecognized()) {
		...
		_instance->setStandardInstance(leftArg, rightArg, prop, _currentMentionSet);
		if (!left->getEntityType().isRecognized())
			_instance->setLeftEntityType(EnglishRelationModel::forcedOrgSym);
		else _instance->setRightEntityType(EnglishRelationModel::forcedOrgSym);

		if (tryToCoerceInstance(left, right, mentionToBeChanged, EntityType::getORGType()))
			return true;

		if (!left->getEntityType().isRecognized())
			_instance->setLeftEntityType(EnglishRelationModel::forcedPerSym);
		else _instance->setRightEntityType(EnglishRelationModel::forcedPerSym);
								 ...
								 float score)
{
	...
	if (second->getMentionType() == Mention::LIST) {
		...
		// ??? just a guess ???
		if (_relationFinderType == ACE &&
			first->getNode()->getHeadPreterm()->getTag() == EnglishSTags::NNS &&
				...
				prop->getArg(0)->getMention(_currentMentionSet)->getEntityType().isRecognized())
			{
				for (int j = 1; j < prop->getNArgs(); j++) {
					...
					if (arg->getType() == Argument::PROPOSITION_ARG &&
						arg->getRoleSym() == EnglishWordConstants::WHERE)
					{
...
void OldRelationFinder::addPartitiveRelations() {
	for (int i = 0; i < _currentMentionSet->getNMentions(); i++) {
		...
		if (ment->getEntityType().isRecognized()) {
			...
			// "[] of them"
			if (_allow_type_coercing && inner_word == EnglishWordConstants::THEM) {
