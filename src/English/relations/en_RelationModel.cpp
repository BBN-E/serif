...
Symbol EnglishRelationModel::forcedOrgSym = Symbol(L"FORCED_ORG");
Symbol EnglishRelationModel::forcedPerSym = Symbol(L"FORCED_PER");

EnglishRelationModel::EnglishRelationModel()
{
	USE_PATTERNS = false;
	USE_METONYMY = false;
...
void EnglishRelationModel::train(char *training_file, char* output_file_prefix)
{
	if (!USE_PATTERNS) {
...
void EnglishRelationModel::trainFromStateFileList(char *training_file, char* output_file_prefix)
{
	if (!USE_PATTERNS) {
		SessionLogger::info("SERIF") << "training on " << training_file << std::endl;
...
EnglishRelationModel::EnglishRelationModel(const char *file_prefix)
{
...
int EnglishRelationModel::findBestRelationType(PotentialRelationInstance *instance) {
...
int EnglishRelationModel::findBestRelationTypeBasic(PotentialRelationInstance *instance) {
...
int EnglishRelationModel::findBestRelationTypeLeftMetonymy(PotentialRelationInstance *instance) {
	int result = 0;
...
int EnglishRelationModel::findBestRelationTypeRightMetonymy(PotentialRelationInstance *instance) {
	int result = 0;
