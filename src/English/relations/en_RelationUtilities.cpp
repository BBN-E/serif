...
//SymbolHash * EnglishRelationUtilities::_unreliabilityIndicators;
...
Symbol EnglishRelationUtilities::WEA = Symbol(L"WEA");
Symbol EnglishRelationUtilities::VEH = Symbol(L"VEH");
Symbol EnglishRelationUtilities::PER_SOC = Symbol(L"PER-SOC");
Symbol EnglishRelationUtilities::PHYS_LOCATED = Symbol(L"PHYS.Located");
Symbol EnglishRelationUtilities::PHYS_NEAR = Symbol(L"PHYS.Near");
Symbol EnglishRelationUtilities::PART_WHOLE_GEOGRAPHICAL = Symbol(L"PART-WHOLE.Geographical");
Symbol EnglishRelationUtilities::PART_WHOLE_SUBSIDIARY = Symbol(L"PART-WHOLE.Subsidiary");
Symbol EnglishRelationUtilities::PART_WHOLE_ARTIFACT = Symbol(L"PART-WHOLE.Artifact");
Symbol EnglishRelationUtilities::ORG_AFF_EMPLOYMENT = Symbol(L"ORG-AFF.Employment");
Symbol EnglishRelationUtilities::ORG_AFF_OWNERSHIP = Symbol(L"ORG-AFF.Ownership");
Symbol EnglishRelationUtilities::ORG_AFF_FOUNDER = Symbol(L"ORG-AFF.Founder");
Symbol EnglishRelationUtilities::ORG_AFF_STUDENT_ALUM = Symbol(L"ORG-AFF.Student-Alum");
Symbol EnglishRelationUtilities::EDUCATIONAL = Symbol(L"Educational");
Symbol EnglishRelationUtilities::ORG_AFF_SPORTS_AFFILIATION = Symbol(L"ORG-AFF.Sports-Affiliation");
Symbol EnglishRelationUtilities::SPORTS = Symbol(L"Sports");
Symbol EnglishRelationUtilities::ORG_AFF_INVESTOR_SHAREHOLDER = Symbol(L"ORG-AFF.Investor-Shareholder");
Symbol EnglishRelationUtilities::ORG_AFF_MEMBERSHIP = Symbol(L"ORG-AFF.Membership");
Symbol EnglishRelationUtilities::ART_USER_OWNER_INVESTOR_MANUFACTURER = Symbol(L"ART.User-Owner-Inventor-Manufacturer");
Symbol EnglishRelationUtilities::GEN_AFF_CITIZEN_RESIDENT_RELIGION_ETHNICITY = Symbol(L"GEN-AFF.Citizen-Resident-Religion-Ethnicity");
Symbol EnglishRelationUtilities::GEN_AFF_ORG_LOCATION = Symbol(L"GEN-AFF.Org-Location");
Symbol EnglishRelationUtilities::PER_SOC_SUBORDINATE = Symbol(L"PER-SOC.Subordinate");

int EnglishRelationUtilities::getRelationCutoff() {
	...
	if (!init) {
		...
		if (relation_cutoff < 0) {
			throw UnexpectedInputException("EnglishRelationUtilities::getRelationCutoff()",
...
std::vector<bool> EnglishRelationUtilities::identifyFalseOrHypotheticalProps(const PropositionSet *propSet,
                                                                      const MentionSet *mentionSet)
{
	...
	for (int k = 0; k < propSet->getNPropositions(); k++) {
		...
		if (prop->getNegation() != 0) {
			// "not just" == true
			if (prop->getAdverb() == 0 ||
				prop->getAdverb()->getHeadWord() != EnglishWordConstants::JUST)
			{
...
bool EnglishRelationUtilities::coercibleToType(const Mention *ment, Symbol type) {
	static int init = 0;
	if (init == 0) {
	...
	if (type == EntityType::getPERType().getName()) {
		if (headword == EnglishWordConstants::HE ||
			headword == EnglishWordConstants::HIM ||
			headword == EnglishWordConstants::HIS ||
			headword == EnglishWordConstants::SHE ||
			headword == EnglishWordConstants::HER ||
			headword == EnglishWordConstants::THEY ||
			headword == EnglishWordConstants::THEM ||
			headword == EnglishWordConstants::US ||
			headword == EnglishWordConstants::WE ||
			headword == EnglishWordConstants::OUR ||
			headword == EnglishWordConstants::I ||
			headword == EnglishWordConstants::ME ||
			headword == EnglishWordConstants::MY ||
			headword == EnglishWordConstants::YOU ||
			headword == EnglishWordConstants::YOUR)
			return true;
		else return WordNet::getInstance()->isPerson(headword);
	} else if (type == EntityType::getLOCType().getName()) {
		if (headword == EnglishWordConstants::HOME ||
			headword == EnglishWordConstants::ABROAD ||
			headword == EnglishWordConstants::OVERSEAS ||
			headword == EnglishWordConstants::HERE ||
			headword == EnglishWordConstants::THERE ||
			headword == oblast_sym)
			return true;
...
void EnglishRelationUtilities::artificiallyStackPrepositions(PotentialRelationInstance *instance)
{
...
Symbol EnglishRelationUtilities::stemPredicate(Symbol word, Proposition::PredType predType)
{
...
Symbol EnglishRelationUtilities::stemWord(Symbol word, Symbol pos) {
...
int EnglishRelationUtilities::getOrgStack(const Mention *mention, Mention **orgs, int max_orgs) {
	...
	for (int i = 0; i < n; i++)
		if (results[i] == EnglishWordConstants::AND)
			return 0;
...
bool EnglishRelationUtilities::isPrepStack(RelationObservation *o) {
...
bool EnglishRelationUtilities::isValidRelationEntityTypeCombo(Symbol validation_type, 
...
bool EnglishRelationUtilities::is2005ValidRelationEntityTypeCombo(const Mention* arg1, const Mention* arg2,
														   Symbol relType )
{
	...
	else if(relType == GEN_AFF_CITIZEN_RESIDENT_RELIGION_ETHNICITY){
		if((arg1_type == PER) ){
	...
	else if(relType == GEN_AFF_ORG_LOCATION){
		if((arg1_type == ORG) ){
...
int EnglishRelationUtilities::calcMentionDist(const Mention *m1, const Mention *m2) {
...
bool EnglishRelationUtilities::distIsLessThanCutoff(const Mention *m1, const Mention *m2) {
...
int EnglishRelationUtilities::getMentionStartToken(const Mention *m1) {
	int start_ment1 = -1;
...
int EnglishRelationUtilities::getMentionEndToken(const Mention *m1){
	int end_ment1 = -1;
...
Symbol EnglishRelationUtilities::mapToTrainedOnSubtype(Symbol subtype) {
...
bool EnglishRelationUtilities::isATEARelationType(Symbol type) {
