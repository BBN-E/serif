...
// expanded in various ways.  This is its new home.  It's possible 
...
EnglishPropositionStatusClassifier::EnglishPropositionStatusClassifier() {
	...
	if (!propStatusPatternFile.empty()) {
		...
		_propStatusPatterns->doNotBlockAnyPatternsBasedOnStatus();
	}
}
...
void EnglishPropositionStatusClassifier::augmentPropositionTheory(DocTheory *docTheory)
{
...
void EnglishPropositionStatusClassifier::augmentPropositionTheoryWithPatterns(DocTheory *docTheory, SentenceTheory *sentTheory,
		...
		for (size_t f = 0; f < match->getNFeatures(); f++) {
			try {
				if (PropositionReturnPFeature_ptr prpf = boost::dynamic_pointer_cast<PropositionReturnPFeature>(match->getFeature(f))) {
					...
					if (SessionLogger::dbg_or_msg_enabled("prop-status")) {
...
void EnglishPropositionStatusClassifier::showPatternDebugMessage(Symbol patternLabel, Symbol status, const Proposition *prop) {
	SessionLogger::dbg("prop-status") << "  [" << patternLabel << "] added status \"" << status << "\" to:";
	std::ostringstream ostr;
	prop->dump(ostr);
	SessionLogger::dbg("prop-status") << "    " << ostr.str();
}


/** Add proposition status labels that can't (easily) be added using patterns.
*/
void EnglishPropositionStatusClassifier::augmentPropositionTheoryExtras(DocTheory *docTheory, SentenceTheory *sentTheory)
{
...
void EnglishPropositionStatusClassifier::propagatePropStatuses(DocTheory *docTheory, SentenceTheory *sentTheory) {
	PropositionSet *propSet = sentTheory->getPropositionSet();
		...
		for (int p = 0; p < propSet->getNPropositions(); p++) {
			...
			if (prop->hasAnyStatus()) {
				if (SessionLogger::dbg_or_msg_enabled("prop-status")) {
					std::ostringstream ostr;
					prop->dump(ostr);
					SessionLogger::dbg("prop-status") << "Attempting to propagate from " << ostr.str();
				}
				...
				if (prop->getPredType() == Proposition::VERB_PRED) {
					for (int j = 0; j < prop->getNArgs(); j++) {	
						...
						if (//arg->getRoleSym() == Argument::SUB_ROLE || 
							...
							arg->getRoleSym() == atSym ||  // "suspect is aimed at building"
							arg->getRoleSym() == forSym ||
							arg->getRoleSym() == fromSym ||	
							arg->getRoleSym() == howSym ||	
							arg->getRoleSym() == onSym ||  // "allegations that Iran has worked on designing"
							...
							if (arg->getType() == Argument::PROPOSITION_ARG) {
								...
								if (SessionLogger::dbg_or_msg_enabled("prop-status")) {
									...
									wss1 << "propagatePropStatuses() with role " << arg->getRoleSym();
									std::wstringstream wss2;
									BOOST_FOREACH(PropositionStatusAttribute status, prop->getStatuses()) {
										wss2 << status.toString() << " ";
									}
									showPatternDebugMessage(wss1.str(), wss2.str(), arg->getProposition());
								}
