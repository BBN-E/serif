...
const Symbol EnglishRuleNameLinker::ATTR_FULLEST = Symbol(L"FullestName");
const Symbol EnglishRuleNameLinker::ATTR_FIRST_MI_LAST = Symbol(L"FirstMILastName");
const Symbol EnglishRuleNameLinker::ATTR_FIRST_LAST = Symbol(L"FirstLastName");
const Symbol EnglishRuleNameLinker::ATTR_LAST = Symbol(L"LastName");
const Symbol EnglishRuleNameLinker::ATTR_MI = Symbol(L"MiddleInitial");
const Symbol EnglishRuleNameLinker::ATTR_MIDDLE = Symbol(L"MiddleName");
const Symbol EnglishRuleNameLinker::ATTR_SUFFIX = Symbol(L"Suffix");
const Symbol EnglishRuleNameLinker::ATTR_NO_DESIG = Symbol(L"NoDesignators");
const Symbol EnglishRuleNameLinker::ATTR_SINGLE_WORD = Symbol(L"SingleWord");
const Symbol EnglishRuleNameLinker::ATTR_ABBREV = Symbol(L"Abbreviation");
const Symbol EnglishRuleNameLinker::ATTR_MILITARY = Symbol(L"Military");

const int EnglishRuleNameLinker::RANK_LEVEL_0 = 0;
const int EnglishRuleNameLinker::RANK_LEVEL_10 = 10;
const int EnglishRuleNameLinker::RANK_LEVEL_20 = 20;
const int EnglishRuleNameLinker::RANK_LEVEL_30 = 30;
const int EnglishRuleNameLinker::RANK_LEVEL_40 = 40;
const int EnglishRuleNameLinker::RANK_LEVEL_50 = 50;
const int EnglishRuleNameLinker::RANK_LEVEL_60 = 60;
const int EnglishRuleNameLinker::RANK_LEVEL_70 = 70;
const int EnglishRuleNameLinker::RANK_LEVEL_80 = 80;
const int EnglishRuleNameLinker::RANK_LEVEL_90 = 90;
const int EnglishRuleNameLinker::RANK_LEVEL_100 = 100;

int EnglishRuleNameLinker::initialTableSize = 5;

EnglishRuleNameLinker::EnglishRuleNameLinker() : _debug(Symbol(L"rule_name_link_debug")) { 
...
EnglishRuleNameLinker::~EnglishRuleNameLinker() {
...
int EnglishRuleNameLinker::linkMention(LexEntitySet *currSolution, 
...
void EnglishRuleNameLinker::cleanUpAfterDocument() { 
	_per_mentions->clear();
	_org_mentions->clear();
	_gpe_mentions->clear();
	_oth_mentions->clear();
...
void EnglishRuleNameLinker::loadAlternateSpellings(Symbol filepath) {
...
void EnglishRuleNameLinker::loadFileIntoTable(Symbol filepath, HashTable * table) {
	boost::scoped_ptr<UTF8InputStream> in_scoped_ptr(UTF8InputStream::build());
	UTF8InputStream& in(*in_scoped_ptr);
	// XXX put error checking here -- SRS
	in.open(filepath.to_string());
...
Symbol EnglishRuleNameLinker::linkPERMention(LexEntitySet *currSolution, Mention *currMention) {
...
Symbol EnglishRuleNameLinker::linkORGMention(LexEntitySet *currSolution, Mention *currMention) {
	HashTable::iterator iter;
	int rank = 0;
	int newRank = 0;
	Symbol entID = Symbol();
...
Symbol EnglishRuleNameLinker::linkGPEMention(LexEntitySet *currSolution, Mention *currMention) {
	HashTable::iterator iter;
	int rank = 0;
	int newRank = 0;
	Symbol entID = Symbol();
...
Symbol EnglishRuleNameLinker::linkOTHMention(LexEntitySet *currSolution, Mention *currMention) {
	HashTable::iterator iter;
	Symbol entID = Symbol();
...
void EnglishRuleNameLinker::mergeMentions(HashTable * source, HashTable * destination,
								   Symbol entID)
{
	HashTable::iterator iter;
...
void EnglishRuleNameLinker::generateOTHVariations(int wordCount, 
...
void EnglishRuleNameLinker::generateGPEVariations(int wordCount,
										   const wchar_t **wordArray,
										   HashTable *attributes)
{
	attributes->clear();
...
void EnglishRuleNameLinker::generateORGVariations(int wordCount,
										   const wchar_t **wordArray,
										   HashTable *attributes)
{
	attributes->clear();
...
void EnglishRuleNameLinker::generatePERVariations(int word_count,
										   const wchar_t **wordArray,
										   HashTable *attributes,
										   HashTable *uniqAttributes)
{
	...
	if (ITEA_LINKING) {
		...
		for (int i = 0; i < word_count; i++) {
			...
			if (word == EnglishWordConstants::LEFT_BRACKET ||
				word == EnglishWordConstants::RIGHT_BRACKET ||
				word == EnglishWordConstants::DOUBLE_LEFT_BRACKET ||
				word == EnglishWordConstants::DOUBLE_RIGHT_BRACKET ||
				word == EnglishWordConstants::LNU ||
				word == EnglishWordConstants::FNU)
			{	
				new_word_count--;
				continue;
			}
...
bool EnglishRuleNameLinker::isReversedCommaName(const wchar_t **wordArray, int word_count) {
	return word_count > 2 && Symbol(wordArray[1]) == EnglishWordConstants::_COMMA_;
}

void EnglishRuleNameLinker::addAcronymAttributes(int wordCount,
										   Symbol *wordArray,
										   HashTable *attributes)
{
	if (!ITEA_LINKING)
...
void EnglishRuleNameLinker::addAttribute(HashTable *table, Symbol key, Symbol value) {
...
void EnglishRuleNameLinker::replaceChar(wstring &str, const wstring &from,
								 const wstring &to)
{
	size_t iter = 0;
...
Symbol EnglishRuleNameLinker::getValueFromTable(HashTable *table, Symbol key) {
	HashTable::iterator iter;
	iter = table->find(key);
	if (iter == table->end()) {
		return Symbol();
...
Mention *EnglishRuleNameLinker::getMentionFromTerminal(LexEntitySet *currSolution, 
...
Symbol EnglishRuleNameLinker::getEntityIDFromTerminal(LexEntitySet *currSolution, 
							   Mention *currMention, const SynNode *terminal) 
{
...
Symbol EnglishRuleNameLinker::checkForContextLinks(LexEntitySet *currSolution, Mention *currMention) {
	if (!ITEA_LINKING)
		return Symbol();
...
Symbol EnglishRuleNameLinker::checkForAKA(LexEntitySet *currSolution, 
							   Mention *currMention) 
{
	...
	while (!node->isPreterminal()) {
		...
		if (next != 0) {
			...
			if (potentialAKA == EnglishWordConstants::IS || potentialAKA == EnglishWordConstants::WAS) {
				next = next->getNextTerminal();
				if (next != 0) 
					potentialAKA = next->getTag();
			}
			if (potentialAKA == EnglishWordConstants::AKA1 || 
				potentialAKA == EnglishWordConstants::AKA2 || 
				potentialAKA == EnglishWordConstants::AKA3 ||
				potentialAKA == EnglishWordConstants::AKA4) 
			{
				next = next->getNextTerminal();
				while (next != 0 && WordConstants::isPunctuation(next->getTag()))
					next = next->getNextTerminal();
				...
				if (entID.is_null() && next != 0 && next->getTag() == EnglishWordConstants::THE) {
					next = next->getNextTerminal();
		...
		if (potentialAKA == EnglishWordConstants::AKA1 || 
			...
			while (prev != 0 && 
				(WordConstants::isPunctuation(prev->getTag()) || prev->getTag() == EnglishWordConstants::THE))
		...
		if (prev != 0 && found_aka) {
			if (prev->getTag() == EnglishWordConstants::IS || prev->getTag() == EnglishWordConstants::WAS)
				prev = prev->getPrevTerminal();
...
Symbol EnglishRuleNameLinker::checkForParenDef(LexEntitySet *currSolution, Mention *currMention) 
{
	if (!currMention->getEntityType().matchesFAC() &&
		!currMention->getEntityType().matchesGPE() &&
		!currMention->getEntityType().matchesPER() &&
	...
	while (!node->isPreterminal()) {
		...
		const SynNode *next = node->getNextTerminal();
		if (next != 0 && next->getTag() == EnglishWordConstants::LEFT_BRACKET) {
			next = next->getNextTerminal();
			...
			if (mention != 0) {
				...
				if (bracket != 0 && bracket->getTag() == EnglishWordConstants::RIGHT_BRACKET) {
					Symbol entID = getEntityIDFromTerminal(currSolution, currMention, next);
					if (!entID.is_null())
						return entID;		
				}
		...
		if (prev != 0 && prev->getTag() == EnglishWordConstants::LEFT_BRACKET &&
			next != 0 && next->getTag() == EnglishWordConstants::RIGHT_BRACKET) {			
			prev = prev->getPrevTerminal();
			Symbol entID = getEntityIDFromTerminal(currSolution, currMention, prev);
			if (!entID.is_null())
				return entID;		
		...
		next = node->getNextTerminal();
		if (next != 0 && next->getTag() == EnglishWordConstants::DOUBLE_RIGHT_BRACKET) {
			next = next->getNextTerminal();
			if (next != 0 && next->getTag() == EnglishWordConstants::LEFT_BRACKET) {
				Mention *mention = getMentionFromTerminal(currSolution, currMention, next);
				if (mention != 0) {
		...
		if (prev != 0 && prev->getTag() == EnglishWordConstants::LEFT_BRACKET &&
			next != 0 && next->getTag() == EnglishWordConstants::RIGHT_BRACKET) {			
			prev = prev->getPrevTerminal();
			if (prev != 0 && prev->getTag() == EnglishWordConstants::DOUBLE_RIGHT_BRACKET) {
				prev = prev->getPrevTerminal();
				Symbol entID = getEntityIDFromTerminal(currSolution, currMention, prev);
				if (!entID.is_null())
					return entID;
