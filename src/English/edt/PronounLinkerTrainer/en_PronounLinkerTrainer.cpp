...
#include "English/edt/PronounLinkerTrainer/en_PronounLinkerTrainer.h"
...
EnglishPronounLinkerTrainer::EnglishPronounLinkerTrainer() : _antPriorWriter(2,100), _hobbsDistWriter(2,25),
	_pronHeadWordWriter(4,250), _pronParWordWriter(2,500)
{
	Guesser::initialize();
//	_debugOut.openFile("c:\\data\\parses.txt");

	// open output files
	char prefix[501];
	if (!ParamReader::getParam("pronlink_model",prefix,									 500))	{
		throw UnexpectedInputException("EnglishPronounLinkerTrainer::EnglishPronounLinkerTrainer()",
...
EnglishPronounLinkerTrainer::~EnglishPronounLinkerTrainer() {
	closeInputFile();
	closeOutputFiles();
}

void EnglishPronounLinkerTrainer::openInputFile(char *trainingFile) {
	if(trainingFile != NULL)
		_trainingSet.openFile(trainingFile);
}

void EnglishPronounLinkerTrainer::closeInputFile() {
	_trainingSet.closeFile();
}

void EnglishPronounLinkerTrainer::openOutputFiles(const char* model_prefix)
{
	char file[501];
...
void EnglishPronounLinkerTrainer::closeOutputFiles() {
...
void EnglishPronounLinkerTrainer::trainModels() {
...
void EnglishPronounLinkerTrainer::trainDocument(CorefDocument *document) {
			...
			if(!WordConstants::isPronoun(mention->getHead()->getHeadWord()))
				...
				for(j=0; document->parses[j]->getRoot() != mentionRoot; j++) {
					if(j > document->parses.length())
						throw InternalInconsistencyException("EnglishPronounLinkerTrainer::trainDocument()", "Pronoun's parse tree not found in parses array!");
				...
				if(antecedent == NULL)
					throw InternalInconsistencyException("EnglishPronounLinkerTrainer::trainDocument()", "Expected antecedent not found.");
...
void EnglishPronounLinkerTrainer::registerPronounLink(Mention *antecedent, const SynNode *antecedentNode,
...
void EnglishPronounLinkerTrainer::writeModels() {
