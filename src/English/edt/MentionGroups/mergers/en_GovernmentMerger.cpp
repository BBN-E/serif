...
EnglishGovernmentMerger::EnglishGovernmentMerger(MentionGroupConstraint_ptr constraints) :
	PairwiseMentionGroupMerger(Symbol(L"EnglishGovernment"), constraints) {}

bool EnglishGovernmentMerger::shouldMerge(const Mention *m1, const Mention *m2, LinkInfoCache& cache) const {
	...
	BOOST_FOREACH(AttributeValuePair_ptr f, features) {
		...
		if ((p = boost::dynamic_pointer_cast< AttributeValuePair<const Proposition*> >(f))) {
			...
			EnglishPreLinker::preLinkGovernment(links, prop, mentionSet);
				...
				links[m2->getIndex()] == m1)
			{
				if (SessionLogger::dbg_or_msg_enabled("MentionGroups_shouldMerge")) {
					...
					SessionLogger::dbg("MentionGroups_shouldMerge") << "GOVERNMENT LINK FOUND";
				}
				return true;
			}
		}
