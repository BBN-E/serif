...
EnglishOrgNameDescMerger::EnglishOrgNameDescMerger(MentionGroupConstraint_ptr constraints) :
	PairwiseMentionGroupMerger(Symbol(L"EnglishOrgNameDesc"), constraints) {}

bool EnglishOrgNameDescMerger::shouldMerge(const Mention *m1, const Mention *m2, LinkInfoCache& cache) const {
	...
	BOOST_FOREACH(AttributeValuePair_ptr f, features) {
		...
		if ((p = boost::dynamic_pointer_cast< AttributeValuePair<const Proposition*> >(f))) {
			...
			EnglishPreLinker::preLinkOrgNameDescs(links, prop, mentionSet);
				...
				links[m2->getIndex()] == m1)
			{
				if (SessionLogger::dbg_or_msg_enabled("MentionGroups_shouldMerge")) {
					...
					SessionLogger::dbg("MentionGroups_shouldMerge") << "ORG NAME DESC FOUND";
				}
				return true;
			}
		}
