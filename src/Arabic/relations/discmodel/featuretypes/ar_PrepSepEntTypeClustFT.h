...
#ifndef AR_PREPSEP_ENTTYPE_CLUST_FT_H
#define AR_PREPSEP_ENTTYPE_CLUST_FT_H
...
class ArabicPrepSepEntTypeClustFT : public P1RelationFeatureType {
public:
	ArabicPrepSepEntTypeClustFT() : P1RelationFeatureType(Symbol(L"prep-sep-enttype-clust")) {}

	virtual DTFeature *makeEmptyFeature() const {
		return _new DTQuadgram2IntFeature(this, SymbolConstants::nullSymbol, SymbolConstants::nullSymbol,
			...
			if(LanguageSpecificFunctions::isPrepPOSLabel(o->getPOS(tok_btwn))){
				...
				for(int i =0; i< 4; i++){
					...
					for(int j =0; j < 4; j++){
						...
						resultArray[nresults++] = _new DTQuadgram2IntFeature(this, state.getTag(),
							o->getMention1()->getEntityType().getName(), 
							o->getMention2()->getEntityType().getName(), 
							o->getToken(tok_btwn), clust1[i], clust2[j]);
					}
				}
				return nresults;
			}
