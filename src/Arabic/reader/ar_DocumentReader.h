...
class ArabicDocumentReader: public DocumentReader {
private:
	friend class ArabicDocumentReaderFactory;
...
public:
	...
	/**
	  ...
	  * document.
	  * The text of the document must be between 
	  * <text></text>
	  * @param stream the input stream to read from.
	  */
	virtual Document* readDocument(InputStream &stream);
	virtual ~ArabicDocumentReader();
...
private:
	ArabicDocumentReader() {};
	
};

class ArabicDocumentReaderFactory: public DocumentReader::Factory {
	virtual DocumentReader *build() { return _new ArabicDocumentReader(); }
};
