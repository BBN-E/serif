...
LocatedString* ArabicDocumentReader::_regions[MAX_DOC_REGIONS];

class ArabicDocumentReader::XMLTag;
using ArabicDocumentReader::XMLTag;

ArabicDocumentReader::~ArabicDocumentReader() { }

Document* ArabicDocumentReader::readDocument(InputStream &stream) {
	XMLTag tag;
	Symbol name;
	...
	if (tag.notFound()) {
		...
		if (tag.notFound()) {
			...
			if (tag.notFound()) {
				throw UnexpectedInputException("ArabicDocumentReader::readDocument()", "DOCNO tag not terminated");
			}
		...
		if (tag.notFound()) {
			throw UnexpectedInputException("ArabicDocumentReader::readDocument()", "DOCID tag not terminated");
		}
		int start_close_docno = tag.getStart();
	...
	while (true) {
		...
		do {
			tag = findOpenTag(text, Symbol(L"P"), start_turn);
			if (tag.notFound()) {
		...
		if (!tag.notFound()) {
			throw UnexpectedInputException("ArabicDocumentReader::readDocument", "too many regions");
		}
...
XMLTag ArabicDocumentReader::findNextXMLTag(const LocatedString *input, int start) {
...
XMLTag ArabicDocumentReader::findCloseTag(const LocatedString *input, Symbol name, int start) {
	XMLTag tag;
	do {
...
XMLTag ArabicDocumentReader::findOpenTag(const LocatedString *input, Symbol name, int start) {
	XMLTag tag;
	do {
		tag = findNextXMLTag(input, start);
		start = tag.getEnd();
...
void ArabicDocumentReader::cleanRegion(LocatedString *region) {
...
Symbol ArabicDocumentReader::cleanFileName(const char* file) const {
	size_t i, j;
...
Symbol ArabicDocumentReader::removeFileExtension(const char* file) const {
	size_t i, j;
	wchar_t* w_file = _new wchar_t[strlen(file)+1];
