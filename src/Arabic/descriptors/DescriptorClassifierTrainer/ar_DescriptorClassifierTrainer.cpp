...
ArabicDescriptorClassifierTrainer::ArabicDescriptorClassifierTrainer() : _priorWriter(2,100),
															 _headWriter(2, 100),
...
ArabicDescriptorClassifierTrainer::~ArabicDescriptorClassifierTrainer() {
	closeOutputFiles();
}

void ArabicDescriptorClassifierTrainer::openOutputFiles(const char* model_prefix)
{
...
void ArabicDescriptorClassifierTrainer::closeOutputFiles() {
	_priorStream.close();
	_headStream.close();
...
void ArabicDescriptorClassifierTrainer::addMention(MentionSet *mentionSet, Mention *mention, Symbol type) {
...
// this was borrowed from ArabicDescriptorClassifier.
// TODO: have some utility functions area?
const SynNode* ArabicDescriptorClassifierTrainer::getFunctionalParentNode(const SynNode* node) {
	if( node == 0 )
		return 0;
...
void ArabicDescriptorClassifierTrainer::writeModels() {
