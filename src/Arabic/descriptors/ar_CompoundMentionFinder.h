...
#ifndef ar_COMPOUND_MENTION_FINDER_H
#define ar_COMPOUND_MENTION_FINDER_H
...
*
*/
class ArabicCompoundMentionFinder : public CompoundMentionFinder {
private:
	// This class should only be instantiated via CompoundMentionFinder::getInstance()
	ArabicCompoundMentionFinder();
	friend struct CompoundMentionFinder::FactoryFor<ArabicCompoundMentionFinder>;
...
private:
	...
	static void _dbgPrintPartitives(const SynNode* part, const SynNode* whole, MentionSet* mentSet, const char* msg ="");


	//try to find partitives
	static Symbol _SYM_of;
	const static bool _printDBGPartitives = false;
