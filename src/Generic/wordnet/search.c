...
#include "Generic/wordnet/setutil.h"
...
void wngrep (char *word_passed, int pos) {
   ...
   while (fgets (line, 1024, inputfile) != NULL) {
      ...
      while ((loc = strstr_getnext ()) != (size_t)-1) {
         if (
...
/* Stucture to keep track of 'relative groups'.  All senses in a relative
   ...
   supported, so if either of a _new set of related senses is already
...
{
     wdtable[nwords] = (char *)_strdup(wd);
     return nwords;
}
     ...
     for (i = 0; i < idx->off_cnt; i++) {
	  ...
	  for (j = 0; j < synset->ptrcount; j++)  
	       if (synset->ptrtyp[j] == HYPERPTR) { 
		    ...
		    for ( ; hypers[l].off != (unsigned long)synset->ptroff[j]; l++)
			 if (hypers[l].off == 0) {
			      hypers[l].off = (unsigned long)synset->ptroff[j];
			      hypers[l].id  =  id++;
			      break;
			 }
...
static void add_relatives(int pos, IndexPtr idx, int rel1, int rel2)
{
    ...
    /* If either of the new relatives are already in a relative group,
       ...
       Otherwise create a _new group and add these 2 senses to it. */
...
"                                                                         \n");
		...
		if (whichsense == ALLSENSES || whichsense == sense + 1) {
			...
			if (offsets[i] == (unsigned long)idx->offset[sense])
			    skipit = 1;
		    }
		    if (skipit != 1) {
		    	offsets[offsetcnt++] = (unsigned long)idx->offset[sense];
...
int get_sense_count(char *searchstr, int dbase)
{
    IndexPtr idx = getindex(searchstr, dbase);
	if (idx != NULL)
		return idx->off_cnt;
	else return 0;
}
