...
int wninit(const char* searchdir)
{
    static int done = 0;
	...
	openerr = do_init(searchdir);
	if (!openerr) {	
	    done = 1;	
	    OpenDB = 1;
	    openerr = morphinit(searchdir);
	}
    }
...
int re_wninit(const char* searchdir)
{
    int openerr;
    char *env;
    ...
    if (!openerr) {
	OpenDB = 1;
	openerr = re_morphinit(searchdir);
    }
