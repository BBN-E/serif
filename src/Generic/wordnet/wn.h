...
#include "Generic/wordnet/wnconsts.h"
#include "Generic/wordnet/wntypes.h"
#include "Generic/wordnet/wnglobal.h"
#include "Generic/wordnet/wnrtl.h"
...
/* Get the sense count for a word */
extern int get_sense_count(char *, int );
...
extern int morphinit(const char *);

/* Close exception list files and reopen */
extern int re_morphinit(const char* searchdir);	
...
/* Return pointer code for pointer type character passed. */
extern int getptrtype(const char *);	

/* Return part of speech code for string passed */
extern int getpos(const char *);		
...
extern size_t strstr_getnext (void);
...
// add for speeding up
extern char *bin_search_memory(char *, char *, int);
