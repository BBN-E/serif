#ifndef AA_DK_ANCHORALLCID_V6_FEATURE_TYPE_H
#define AA_DK_ANCHORALLCID_V6_FEATURE_TYPE_H
...
class AADKAnchorAllCidV6FeatureType : public EventAAFeatureType {
private:
	typedef std::pair<Symbol, int> SymIntPair;

public:
	AADKAnchorAllCidV6FeatureType() : EventAAFeatureType(Symbol(L"aa-anchorallcid-v6")) {}

	virtual DTFeature *makeEmptyFeature() const {
		return _new DTQuadgramIntFeature(this, SymbolConstants::nullSymbol, SymbolConstants::nullSymbol, SymbolConstants::nullSymbol, SymbolConstants::nullSymbol, -1);
	}
	...
	virtual int extractFeatures(const DTState &state, DTFeature **resultArray) const
	{
		...
		for(set<int>::iterator it=anchorAllSubCids.begin(); it!=anchorAllSubCids.end(); it++) {
			resultArray[featureIndex++] = _new DTQuadgramIntFeature(this, state.getTag(), eventType, candidateType, subSym, (*it));
		}
		...
		for(set<int>::iterator it=anchorAllObjCids.begin(); it!=anchorAllObjCids.end(); it++) {
			resultArray[featureIndex++] = _new DTQuadgramIntFeature(this, state.getTag(), eventType, candidateType, objSym, (*it));
		}
