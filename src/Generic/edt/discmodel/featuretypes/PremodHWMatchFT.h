...
#ifndef PREMODANDHWMATCH_FT_H
#define PREMODANDHWMATCH_FT_H
...
/* this feature tries to capture linkage between [Democrat] and [the Democratic party]
 * where 'Democratic' is a pre-modifier of the nominal mention.
 * TODO: make it work with the abbreviation table.
 */
class PremodHWMatchFT : public DTCorefFeatureType {
public:
	PremodHWMatchFT() : DTCorefFeatureType(Symbol(L"premod-hw-match")) {}
								...
								DTFeature **resultArray) const
	{
		...
		notInHWMap = o->getPremodMentionMapper();
		...
		mentNotInHWArray = notInHWMap->get(ment->getUID());
		if (mentNotInHWArray != NULL) {
			mentNotInHeadWords = (*mentNotInHWArray)->getArray();
			n_ment_not_hw_words = (*mentNotInHWArray)->getLength();
			SerifAssert (n_ment_not_hw_words <= MAX_SENTENCE_TOKENS /*&& n_ment_not_hw_words>0 */);
		}
		...
		for (int m = 0; m < entity->getNMentions(); m++) {
			...
			entityNotInHWArray = notInHWMap->get(entity->getMention(m));
			if (entityNotInHWArray != NULL) {
				...
				SerifAssert (n_ent_not_hw_words <= MAX_SENTENCE_TOKENS /*&& n_ent_not_hw_words>0*/);
			}
			...
			if (mentNotInHWArray != NULL) {
				for (int i=0; i< n_ment_not_hw_words; i++) {
					if(n_feat>MAX_FEATURES_PER_EXTRACTION-30) return n_feat;
...
mutable const MentionSymArrayMap *HWMap, *notInHWMap;
