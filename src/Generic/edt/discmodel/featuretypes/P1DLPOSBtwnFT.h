...
#ifndef P1DL_POS_BTWN_FT
#define P1DL_POS_BTWN_FT
...
class P1DLPOSBtwnFT : public DTCorefFeatureType {
public:
	P1DLPOSBtwnFT() : DTCorefFeatureType(Symbol(L"p1dl-posbtwn")) {}
								...
								DTFeature **resultArray) const
	{
		...
		for (int i = 0; i < ent->getNMentions(); i++) {	
			...
			if(ment->getSentenceNumber() == linkment->getSentenceNumber()){
				...
				else if( (wcslen(result_sym.to_string()) < 2)){
				}
				else if (nresults >= (DTFeatureType::MAX_FEATURES_PER_EXTRACTION-2)){
					SessionLogger::warn("DT_feature_limit") 
						<<"P1DLPOSBtwnFT discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
					break;
				}else{
					resultArray[nresults++] = _new DTQuadgramFeature(this, state.getTag(), result_sym, 
						linkment->getEntityType().getName(), menttype);
					resultArray[nresults++] = _new DTQuadgramFeature(this, state.getTag(), result_sym, 
							linkment->getHead()->getHeadWord(), 
