...
#ifndef EL_DISTANCE_FEATURE_TYPE_H
#define EL_DISTANCE_FEATURE_TYPE_H
...
class ELDistanceFeatureType : public EventLinkFeatureType {
public:
	ELDistanceFeatureType() : EventLinkFeatureType(Symbol(L"distance")) {}
								...
								DTFeature **resultArray) const
	{
		...
		if (difference == 0) {
			resultArray[0] = _new DTTrigramFeature(this, state.getTag(), 
							v1->getEventType(), Symbol(L"same-sentence"));
		} else if (difference == 1) {
			resultArray[0] = _new DTTrigramFeature(this, state.getTag(), 
							v1->getEventType(), Symbol(L"adjacent-sentence"));
		} else if (difference < 5) {
			resultArray[0] = _new DTTrigramFeature(this, state.getTag(), 
							v1->getEventType(), Symbol(L"within-5-sentences"));
		} else {
			resultArray[0] = _new DTTrigramFeature(this, state.getTag(), 
							v1->getEventType(), Symbol(L"beyond-5-sentences"));
		}
