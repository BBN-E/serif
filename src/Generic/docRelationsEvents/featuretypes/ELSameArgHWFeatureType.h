...
#ifndef EL_SAME_ARG_HW_FEATURE_TYPE_H
#define EL_SAME_ARG_HW_FEATURE_TYPE_H
...
class ELSameArgHWFeatureType : public EventLinkFeatureType {
public:
	ELSameArgHWFeatureType() : EventLinkFeatureType(Symbol(L"same-arg-headword")) {}
								...
								DTFeature **resultArray) const
	{
		...
		for (int i = 0; i < v1->getNArgs(); i++) {
			for (int j = 0; j < v2->getNArgs(); j++) {
				if (v1->getNthArgRole(i) == v2->getNthArgRole(j) &&
					v1->getNthArgMention(i)->getNode()->getHeadWord() == 
					v2->getNthArgMention(i)->getNode()->getHeadWord())
				{
					if (count >= DTFeatureType::MAX_FEATURES_PER_EXTRACTION) {
						...
						resultArray[count++] = _new DTTrigramFeature(this, state.getTag(), 
							v1->getEventType(),	v1->getNthArgRole(i));
					}
				}
			}
		}
		if (discard)  {
			SessionLogger::warn("DT_feature_limit") 
				<<"ELSameArgHWFeatureType discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
		}
		return count;
	}
};
