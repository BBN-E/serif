...
#ifndef EL_ARG_CONFLICT_RED_FEATURE_TYPE_H
#define EL_ARG_CONFLICT_RED_FEATURE_TYPE_H
...
class ELArgConflictRedFeatureType : public EventLinkFeatureType {
public:
	ELArgConflictRedFeatureType() : EventLinkFeatureType(Symbol(L"arg-conflict-red")) {}
			...
			if (!same_found) {
				if (count >= DTFeatureType::MAX_FEATURES_PER_EXTRACTION) {
						SessionLogger::warn("DT_feature_limit") 
							<<"ELArgConflictRedFeatureType discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
						break;
					}else{
						resultArray[count++] = _new DTTrigramFeature(this, state.getTag(), 
							v1->getReducedEventType(),	differentArray[k]);
				}
			}
		}
