...
//look for structure ((NP (N ment1) (NP (N Ment2)))
class NomModFT : public P1RelationFeatureType {
public:
	/*
	...
	*/
	NomModFT() : P1RelationFeatureType(Symbol(L"nom-mod")) {}
	static bool hasNomModForm(const SynNode* coveringnode, const SynNode* node1, 
		const SynNode* node2)
	{
		...
		for(int i=0; i< coveringnode->getNChildren(); i++){
			if(coveringnode->getChild(i)->isAncestorOf(node1)){
				...
				const SynNode* np = coveringnode->getChild(i+1);
				if(!((np->getTag() == STags::getNP()) || (np->getTag() == STags::getNPA()))){
					return false;
				}
				if(node2->getHeadPreterm() == np->getHeadPreterm()){
					return true;
				}
				return false;
			}
								...
								DTFeature **resultArray) const
	{
		...
		if(m1start < m2start){
			...
			if(coveringNode !=0){
				if(hasNomModForm(coveringNode, ment1NodeP2, ment2NodeP2)){
					...
					//			ment1->getNode()->getHeadWord(), 
					//			ment2->getNode()->getHeadWord() );
					return 1;
				}
			}
		}
			...
			if(coveringNode !=0){
				if(hasNomModForm(coveringNode, ment2NodeP2, ment1NodeP2)){
					resultArray[0] = _new DTTrigramFeature(this, state.getTag(), 
								  o->getMention1()->getEntityType().getName(), 
								  o->getMention2()->getEntityType().getName());
					//resultArray[1] = _new DTTrigramFeature(this, state.getTag(), 
