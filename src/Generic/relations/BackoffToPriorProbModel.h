...
#ifndef BACKOFFTOPRIORPROBMODEL_H
#define BACKOFFTOPRIORPROBMODEL_H
...
class BackoffToPriorProbModel: public RelationProbModel {
public:
	static const int LOG_OF_ZERO = -10000;

	// read in a model file
	BackoffToPriorProbModel( UTF8InputStream &stream ) : _historySize(historySize),
		_observedEvents(historySize+1, INIT_TABLE_SIZE) 
	{ 
		_dummySym = Symbol(L"DUMMY");
		_nGramLambdas =_new NgramScoreTable*[_historySize];
		_nGramProbs =_new NgramScoreTable*[_historySize+1];
		_initializeFormula();
		// fill lambdas and probs tables from file
		for (int i = _historySize-1; i >=0; i--)
			_nGramLambdas[i] =_new NgramScoreTable(i+1, stream);
		for (int i = _historySize; i >= 0; i--)
			_nGramProbs[i] =_new NgramScoreTable(i+1, stream);
		_ngram =_new Symbol[_historySize+1];
		_uniqueMultipliers =_new float[_historySize];
	...
	// create empty tables
	BackoffToPriorProbModel() : _historySize(historySize), 
		_observedEvents(historySize+1, INIT_TABLE_SIZE) 
	{

		_dummySym = Symbol(L"DUMMY");
		_nGramLambdas =_new NgramScoreTable*[_historySize>0 ? _historySize : 1];
		_nGramProbs =_new NgramScoreTable*[_historySize>0 ? _historySize+1 : 1];

		for (int i = _historySize-1; i >=0; i--)
			_nGramLambdas[i] =_new NgramScoreTable(i+1, INIT_TABLE_SIZE);
		for (int i = _historySize; i >= 0; i--) 
			_nGramProbs[i] =_new NgramScoreTable(i+1, INIT_TABLE_SIZE);
		_ngram =_new Symbol[_historySize+1];
	...
	double getProbability( Symbol *features ) const { 
		...
		double *probs =_new double[_historySize+1];
		for (i = 0; i < _historySize+1; i++) {
			probs[i] = _nGramProbs[i]->lookup(_ngram);
		}

		// find the lambdas for each history
		double *lambdas =_new double[_historySize];
	...
	double getLambdaForFullHistory(Symbol *features) {
		...
		return _nGramLambdas[_historySize-1]->lookup(_ngram + 1);
	}
	...
	void deriveModel(NgramScoreTable *rawData) {
		...
		NgramScoreTable**  histories          =_new NgramScoreTable*[_historySize+1];
		NgramScoreTable**  transitions        =_new NgramScoreTable*[_historySize+1];
		NgramScoreTable**  uniqueTransitions  =_new NgramScoreTable*[_historySize];
		NgramScoreTable**  storage            =_new NgramScoreTable*[_historySize]; // [xx] never used??
		transitions[_historySize] = rawData; // top transitions is the rawData
		int size = transitions[_historySize]->get_size();
		histories[_historySize] =_new NgramScoreTable((_historySize > 0) ? _historySize : 1, size);
		int i;
		for (i = 0; i < _historySize; i++) {
			transitions[i] =_new NgramScoreTable(i+1, INIT_TABLE_SIZE);
			// histories[0] is faked
			histories[i] =_new NgramScoreTable(((i>0) ? i : 1), INIT_TABLE_SIZE);
		}
		...
		for (iter = transitions[_historySize]->get_start(); 
			iter != transitions[_historySize]->get_end(); ++iter) 
		{
			count = (*iter).second;
			//   cout << "Ngram for histories and transitions: "; 
			int j = 0;
			...
			for (j = 0; j < (_historySize+1); j++) {
				if (j == 0)
					histories[j]->add(&_dummySym, count);
				else
		...
		for (i = _historySize; i >= 0; i--) {
			for (iter = transitions[i]->get_start(); 
				iter != transitions[i]->get_end(); ++iter) {
					...
					history_count = histories[i]->lookup(((i>0) ? _ngram : &_dummySym));
		...
		for (i = _historySize; i > 0; i--) { // note history[0] will be untouched
			for (iter = histories[i]->get_start(); 
				iter != histories[i]->get_end(); ++iter) {
					...
					//              << unique_multipliers[i-1] << " * " << unique_count << ") = ";
		...
		for (i = 0; i < _historySize; i++) {
			...
			delete histories[i];
			delete uniqueTransitions[i];
		}
		delete histories[_historySize];
		delete[] histories;
		delete[] transitions;
	...
	void printFormula(UTF8OutputStream &out) const{
		out << "BackoffToPriorProbModel: " << _formula;
	}
...
private:
	...
	void _initializeFormula () {
		string *probs =_new string[_historySize+1];
		string *lambdas =_new string[_historySize];
		int i = 0;
		for (i = 0; i < _historySize+1; i++) {
			char buf[50];
			sprintf(buf, "p(f0");
		...
		for (i = 0; i < _historySize; i++) {
			...
			for (int j = 1; j < i+1; j++)
				sprintf(buf, "%s, f%d", buf, j+1);
			sprintf(buf, "%s)", buf);
			lambdas[i] = string(buf);
		}
		char bigbuf[200];
		sprintf(bigbuf, "%s", probs[0].c_str());
		// the conversions to string before pulling back to char* avoid recursion
		for (i = 0; i < _historySize; i++) {
