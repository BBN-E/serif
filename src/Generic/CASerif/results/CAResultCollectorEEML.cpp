...
#include "eeml/GroupFnGuesser.h"
//#include "common/StringTransliterator.h"

using namespace std;

#define PRINT_OUT_GENERIC_EE_RELATIONS true
#define PRINT_OUT_GENERIC_RELATIONS true
#define PRINT_EVERYTHING_AS_SPECIFIC false

void CAResultCollectorEEML::loadDocTheory(DocTheory* theory) {
...
void CAResultCollectorEEML::produceOutput(const wchar_t *output_dir,
									      const wchar_t *doc_filename)
{
	...
	if (_entitySet != NULL) {
		...
		for (int i = 0; i < ents; i++) {
			...
			// "false" needs to be replaced with a boolean indicating whether the 
			// entity is Group_FN or not
			...
			/*// cycle through all CorrectMentions and print out any that hasn't been printed
			if (ce != NULL) {
				size_t i;
				for (i = 0; i < ce->getNMentions(); i++) {
					CorrectMention *cm = ce->getMention(i);
					if (cm->printed) continue;
					_printCorrectMention(stream, cm, ent);
					cm->printed = true;
				}
			}*/
	...
	if (_eventSet != NULL) {
		...
		for (int i = 0; i < n_events; i++) {
			...
			for (int j = 0; j < n_ee_relations; j++) {
				...
				if (!isEntityPrinted[eeRelation->getEntityID()])
					continue;
	...
	if (_relationSet != NULL) {
		...
		for (int i = 0; i < n_rels; i++) {
			...
			if (!PRINT_OUT_GENERIC_RELATIONS &&
				!PRINT_EVERYTHING_AS_SPECIFIC && 
				((leftCE && leftCE->isGeneric()) ||
...
const wchar_t* CAResultCollectorEEML::_convertMentionTypeToEEML(Mention* ment)
{
	...
	if (cm != NULL)
		return cm->getCorrectMentionType().to_string();
	...
	switch (type) {
		...
		case Mention::LIST:
			return L"Nominal";
		default:
			return L"WARNING BAD MENTION TYPE";
	}
}
...
void CAResultCollectorEEML::_printEEMLDocumentHeader(UTF8OutputStream& stream,
...
void CAResultCollectorEEML::_printEEMLEntityHeader(UTF8OutputStream& stream, 
											Entity* ent, 
											bool isGeneric, 
											bool isGroupFN,
											const wchar_t* doc_name)
...
void CAResultCollectorEEML::_printEEMLAttributeHeader(UTF8OutputStream& stream) 
{
	stream << L"        <entity_attributes>\n";
}

void CAResultCollectorEEML::_printEEMLAttributeFooter(UTF8OutputStream& stream) 
{
	stream << L"        </entity_attributes>\n";
}

void CAResultCollectorEEML::_printCorrectMention(UTF8OutputStream& stream,
									   CorrectMention* cm,
									   Entity* ent)
...
void CAResultCollectorEEML::_printEEMLMention(UTF8OutputStream& stream,
									   Mention* ment,
									   Entity* ent)
{
	...
	const wchar_t *eemlMentionType = _convertMentionTypeToEEML(ment);
	if (eemlMentionType == NULL) 
		return;
...
void CAResultCollectorEEML::_printEEMLNameAttribute(UTF8OutputStream& stream,
											 Mention* ment)
{
	stream << L"          <name>\n";
...
void CAResultCollectorEEML::_printEEMLMentionExtent(UTF8OutputStream& stream,
											 const SynNode* node,
											 int sentNum)
{
	stream << L"              <charseq>\n";
	...
	int end = _tokenSequence[sentNum]->getToken(endTok)->getEndEDTOffset().value() + 1; 
	stream << L"                <start>" << start << L"</start><end>" << end <<  L"</end></charseq>\n";
}

void CAResultCollectorEEML::_printCorrectMentionExtent(UTF8OutputStream& stream,
												   CorrectMention *cm,
												   int sentNum)
{
	stream << L"              <charseq>\n";
...
void CAResultCollectorEEML::_printCorrectMentionHeadExtent(UTF8OutputStream& stream,
												   CorrectMention *cm,
												   int sentNum)
{
	stream << L"              <charseq>\n";
...
void CAResultCollectorEEML::_printEEMLNodeText(UTF8OutputStream& stream, 
		const SynNode* node,
		int sentNum) 
{
	int startTok = node->getStartToken();
...
void CAResultCollectorEEML::_printCorrectMentionText(UTF8OutputStream& stream, 
		CorrectMention *cm,
		int sentNum) 
{
	int startTok = cm->getStartToken();
...
void CAResultCollectorEEML::_printCorrectMentionHeadText(UTF8OutputStream& stream, 
		CorrectMention *cm,
		int sentNum) 
{
	int startTok = cm->getHeadStartToken();
...
void CAResultCollectorEEML::_printEEMLRelationHeader(UTF8OutputStream& stream, 
											Relation* rel,  
											const wchar_t* doc_name)
{
...
void CAResultCollectorEEML::_printEEMLEventHeader(UTF8OutputStream& stream, 
											Event* event,  
											bool isGeneric,
...
void CAResultCollectorEEML::_printEEMLEventMention(UTF8OutputStream& stream, 
...
void CAResultCollectorEEML::_printEEMLEERelationHeader(UTF8OutputStream& stream, 
...
void CAResultCollectorEEML::_printEEMLEERelationMention(UTF8OutputStream& stream, 
													  EventMention* eventMention, 
													  ...
													  EventEntityRelation* eeRelation)
{
	...
	if (!_isValidMention[mention->getUID()]) {
		...
		logger.beginWarning();
		logger << "CAResultCollectorEEML::_printEEMLEERelationMention: Invalid mention referenced ";
		logger << "in output:\n";
		logger << mention->getNode()->toDebugTextString().c_str() << "\n";
	...
	stream << L"        <relation_mention ID=\"EER" << eeRelation->getUID() << "-" 
		<< eventMention->getUID().toInt() << "-" << mention->getUID() << "\">\n";
...
void CAResultCollectorEEML::_printEEMLRelMention(UTF8OutputStream& stream,
									   RelMention* ment,
									   Relation* rel)
{
	...
	if (!_isValidMention[left->getUID()]) {
		SessionLogger &logger = *SessionLogger::logger;
		logger.beginWarning();
		logger << "CAResultCollectorEEML::_printEEMLRelMention: Invalid mention referenced ";
		logger << "in output:\n";
		logger << left->getUID() << " -- ";
		logger << left->getNode()->toDebugTextString().c_str() << "\n";
		...
		logger << "replaced with: " << left->getUID() << "\n";
	}
	if (!_isValidMention[right->getUID()]) {
		...
		logger << "in output:\n";
		logger << right->getUID() << " -- ";
		logger << right->getNode()->toDebugTextString().c_str() << "\n";
		...
		logger << "replaced with: " << right->getUID() << "\n";
	}
...
void CAResultCollectorEEML::_printEEMLEERelationFooter(UTF8OutputStream& stream)
{
	stream << L"      </relation_mentions>\n";
	stream << L"    </relation>\n\n";
}
void CAResultCollectorEEML::_printEEMLEntityFooter(UTF8OutputStream& stream)
{
	stream << L"    </entity>\n\n";
}

void CAResultCollectorEEML::_printEEMLRelationFooter(UTF8OutputStream& stream)
{
	stream << L"      </relation_mentions>\n";
	stream << L"    </relation>\n\n";
}

void CAResultCollectorEEML::_printEEMLDocumentFooter(UTF8OutputStream& stream)
{
	stream << L"  </document>\n";
	stream << L"</source_file>\n";
}

void CAResultCollectorEEML::_printEEMLEventFooter(UTF8OutputStream& stream)
{
	stream << L"    </entity>\n\n";
}
bool CAResultCollectorEEML::_isTopMention(Mention* ment, EntityType type)
{
	if (ment->getParent() == 0)
		return true;
	...
	if (ent == 0) {
		...
		err.append(")\n");
		throw InternalInconsistencyException("CAResultCollectorEEML::_isTopMention()",
			(char*)err.c_str());
...
// a name!
bool CAResultCollectorEEML::_isNestedInsideOfName(Mention* ment)
{
...
bool CAResultCollectorEEML::_isNestedDriver(const SynNode* node, int sentNum)
{
...
bool CAResultCollectorEEML::_isSecondPartOfAppositive(Mention* ment)
{
...
bool CAResultCollectorEEML::_isGPEModifierOfPersonGPE(Mention* ment)
{
	return false;
}
...
bool CAResultCollectorEEML::_isNameNotInHeadOfParent(Mention* ment, EntityType type)
{
	if (ment->mentionType != Mention::NAME)
		return false;
	const SynNode* node = ment->node;
	...
	if (parent != 0 && parent->hasMention()) {		
		...
		if (parMent->isPopulated()) {
			...
			Entity* ent = _entitySet->getEntityByMention(ment->getUID(), type);
			if (ent == 0)
				throw InternalInconsistencyException("CAResultCollectorEEML::_isNameNotInHeadOfParent()",
...
const SynNode* CAResultCollectorEEML::_getEDTHead(Mention* ment)
{
	if (ment == 0)
		throw InternalInconsistencyException("CAResultCollectorEEML::_getEDTHead()",
...
bool CAResultCollectorEEML::_isPluralEntity(Entity *ent) {
