...
bool ICEWSActorInfo::_initialized = false;
ICEWSActorInfo_ptr ICEWSActorInfo::_actorInfo = ICEWSActorInfo_ptr();

struct ICEWSActorInfo::DictActorRow {
...
struct ICEWSActorInfo::DictAgentRow {
...
public:
	...
	const DictActorRow& saveDictActorRow(Symbol db_name, DatabaseConnection::RowIterator &row, DictActorRow& result) const {
		...
		result.country_id = (row.isCellNull(4)) ? CountryId() : CountryId(row.getCellAsInt32(4,0), db_name);
	...
	const DictActorRow& getDictActorRow(ActorId target) {
		...
		if (it != actorRows.end()) {
			...
			DatabaseConnection_ptr icews_db(ICEWSDB::getDb(target));
			for (DatabaseConnection::RowIterator row = icews_db->iter(query); row!=icews_db->end(); ++row) {
				if (actorRows.size() >= Cache::MAX_SIZE)
					actorRows.clear(); 
				return saveDictActorRow(target.getDbName(), row, actorRows[target]);
			}
	...
	void loadActorRows(const ActorId::HashSet& actorIds) {
		...
		for (DatabaseConnectionMap::iterator i = dbs.begin(); i != dbs.end(); ++i) {
			for (DatabaseConnection::RowIterator row = i->second->iter(query); row!=i->second->end(); ++row) {
				ActorId actorId(row.getCellAsInt32(5), i->first);
				saveDictActorRow(i->first, row, actorRows[actorId]);
			}
		}
	}
	...
	AgentId getDefaultPersonAgentId() {
		// defaultPersonAgentId is initialized by this function
		if (agentRows.size()==0) readAgentTable();
		return defaultPersonAgentId;
	}

	Symbol getDefaultPersonAgentCode() {
		// defaultPersonAgentCode is initialized by this function
		if (agentRows.size()==0) readAgentTable();
...
private:
	...
	void readActorIdForCountryTable() {
		...
		for (DatabaseConnectionMap::iterator i = dbs.begin(); i != dbs.end(); ++i) {
			for (DatabaseConnection::RowIterator row = i->second->iter(query); row!=i->second->end(); ++row) {
				ActorId actorId(row.getCellAsInt32(0), i->first);
				CountryId countryId(row.getCellAsInt32(1), i->first);
	...
	void readAgentTable() {
		...
		for (DatabaseConnectionMap::iterator i = dbs.begin(); i != dbs.end(); ++i) {
			for (DatabaseConnection::RowIterator row = i->second->iter(query); row!=i->second->end(); ++row) {
				...
				if (agentRow.name == L"Citizen") {
					defaultPersonAgentId = agentId;
					defaultPersonAgentCode = agentRow.code;
				}
			}
		...
		if (defaultPersonAgentId.isNull()) {
			throw UnexpectedInputException("ICEWSActorInfo::readAgentTable", "No default person agent ('Citizen') in dictionary");
		}
	}
...
ICEWSActorInfo::~ICEWSActorInfo() {
	delete _cache;
}

ActorInfo_ptr ICEWSActorInfo::getICEWSActorInfo() {

	// Throw an error if we're using AWAKE
	if (ParamReader::isParamTrue("use_awake_db_for_icews"))
		throw InternalInconsistencyException("ICEWSActorInfo::getICEWSActorInfo", 
				"Calling getICEWSActorInfo() is illegal if use_awake_db_for_icews=true");
	
	if (!_initialized) {
		_actorInfo = boost::make_shared<ICEWSActorInfo>();
		_initialized = true;
	}
	return _actorInfo;
}

std::vector<ActorId> ICEWSActorInfo::getAssociatedActorIds(ActorId target, const char *date) {
...
std::vector<CountryId> ICEWSActorInfo::getAssociatedCountryIds(ActorId target, const char *date) {
...
std::vector<ActorId> ICEWSActorInfo::getAssociatedLocationActorIds(ActorId target, const char *date) {
	return getAssociatedCountryActorIds(target, date);
}

std::vector<ActorId> ICEWSActorInfo::getAssociatedCountryActorIds(ActorId target, const char *date) {
...
std::vector<SectorId> ICEWSActorInfo::getAssociatedSectorIds(ActorId target, const char *date) {
...
std::vector<Symbol> ICEWSActorInfo::getAssociatedSectorCodes(ActorId target, const char *date, int min_frequency) {
...
std::wstring ICEWSActorInfo::getSectorName(Symbol sectorCode) {
	std::ostringstream query;
	query << "SELECT name from dict_sectors where code=" << DatabaseConnection::quote(sectorCode.to_string());
	DatabaseConnectionMap dbs = ICEWSDB::getNamedDbs();
	for (DatabaseConnectionMap::iterator i = dbs.begin(); i != dbs.end(); ++i) {
		for (DatabaseConnection::RowIterator row = i->second->iter(query); row!=i->second->end(); ++row) {
			// Current assumption is that sector codes are consistent across databases; return the first one we find
...
std::wstring ICEWSActorInfo::getSectorName(SectorId target) {
	std::ostringstream query;
...
ActorId ICEWSActorInfo::getActorByCode(Symbol actorCode) {
	std::ostringstream query;
	query << "SELECT actor_id from dict_actors where unique_code=" << DatabaseConnection::quote(actorCode.to_string());
	DatabaseConnectionMap dbs = ICEWSDB::getNamedDbs();
	for (DatabaseConnectionMap::iterator i = dbs.begin(); i != dbs.end(); ++i) {
		for (DatabaseConnection::RowIterator row = i->second->iter(query); row!=i->second->end(); ++row) {
			// Current assumption is that actor codes are unique across databases; return the first one we find
			return ActorId(row.getCellAsInt32(0), i->first);
		}
	}
	return ActorId();
}

std::wstring ICEWSActorInfo::getActorName(ActorId target) {
	if (target.isNull()) return L"UNKNOWN-ACTOR";
	return _cache->getDictActorRow(target).name;
}

std::wstring ICEWSActorInfo::getAgentName(AgentId target) {
	if (target.isNull()) return L"UNKNOWN-AGENT";
	return _cache->getDictAgentRow(target).name;
}

Symbol ICEWSActorInfo::getActorCode(ActorId target) {
	if (target.isNull()) return Symbol(L"UNKNOWN-ACTOR");
	return _cache->getDictActorRow(target).code;
}

Symbol ICEWSActorInfo::getAgentCode(AgentId target) {
	if (target.isNull()) return Symbol(L"UNKNOWN-AGENT");
	return _cache->getDictAgentRow(target).code;
}

bool ICEWSActorInfo::isACountry(ActorId target) {
	if (target.isNull()) return false;
	return _cache->getDictActorRow(target).is_country;
}

bool ICEWSActorInfo::isAnIndividual(ActorId target) {
	if (target.isNull()) return false;
	return _cache->getDictActorRow(target).is_individual;
}

CountryId ICEWSActorInfo::getCountryId(ActorId target) {
	if (target.isNull()) return CountryId();
	return _cache->getDictActorRow(target).country_id;
}

bool ICEWSActorInfo::isRestrictedToCountryActors(AgentId target) {
	if (target.isNull()) return false;
	return _cache->getDictAgentRow(target).restricted_to_country_actors;
}

std::vector<Symbol> ICEWSActorInfo::getAssociatedSectorCodes(AgentId target) {
	return _cache->getDictAgentRow(target).associatedSectorCodes;
}

AgentId ICEWSActorInfo::getDefaultPersonAgentId() {
	return _cache->getDefaultPersonAgentId();
}

Symbol ICEWSActorInfo::getDefaultPersonAgentCode() {
	return _cache->getDefaultPersonAgentCode();
}

ActorId ICEWSActorInfo::getActorIdForCountry(CountryId country_id) {
	return _cache->getActorIdForCountry(country_id);
}

void ICEWSActorInfo::preCacheActorInfo(const ActorId::HashSet& actorIds) {
	_cache->loadActorRows(actorIds);
}

bool ICEWSActorInfo::mightBeALocation(ActorId target) {
...
Symbol ICEWSActorInfo::getIsoCodeForActor(ActorId actor_id) 
{
	DatabaseConnection_ptr icews_db(ICEWSDB::getDb(actor_id));
	std::ostringstream query;
	...
	for (DatabaseConnection::RowIterator row = icews_db->iter(query); row!=icews_db->end(); ++row) 
	{
		...
		while (row != icews_db->end()) ++row; // MySQL wants us to clear out the query before moving on
		return s;
	}
	return Symbol();
}
