...
namespace {
	...
	void throwSexpError(const Sexp* sexp, const char* reason) {
		...
		throw UnexpectedInputException("Pattern::throwSexpError()", error.str().c_str());
	}
...
void ICEWSEventType::readEventTypesFromDatabase() {
	...
	for (DatabaseConnectionMap::iterator i = dbs.begin(); i != dbs.end(); ++i) {
		for (DatabaseConnection::RowIterator row = i->second->iter(query); row!=i->second->end(); ++row) {
			ICEWSEventTypeId id(row.getCellAsInt32(0), i->first);
...
ICEWSEventType::ICEWSEventType(const Sexp* sexp): _discardEvents(false) {
	...
	for (Sexp* child=sexp->getSecondChild(); child; child=child->getNext()) {
		...
		} else if (sym == REQUIRED_ROLE_SYM) {
			if ((child->getNumChildren() != 2) || (!child->getSecondChild()->isAtom()))
				throwSexpError(child, "Bad required role field");
			_requiredRoles.push_back(stripQuotes(child->getSecondChild()->getValue()));
	...
	if (_roles.empty()) {
		...
		_roles.push_back(LOCATION_SYM);
	}
	if (_requiredRoles.empty()) {
		if (ParamReader::getRequiredTrueFalseParam("require_source_and_target_for_icews_events")) {
			// This parameter essentially allows a hard-coded special override for protest events;
			//   this is important because this is how the deployed ICEWS version works
			if (ParamReader::getOptionalTrueFalseParamWithDefaultVal("allow_source_only_icews_protest_events", false) && _eventGroup == Symbol(L"14")) {
				_requiredRoles.push_back(SOURCE_SYM);
			} else {
				_requiredRoles.push_back(SOURCE_SYM);
				_requiredRoles.push_back(TARGET_SYM);
			}
		}
	}
	// Since an empty list is automatically populated with SOURCE/TARGET, we need some way to say, no,
	//   truly, we meant no roles are required.
	if (_requiredRoles.size() == 1 && _requiredRoles.at(0) == NONE_SYM) {
		_requiredRoles.pop_back();
	}
	// Look up id if it was not specified.
	if (_id.isNull() && !explicit_null_id && !_discardEvents && !ParamReader::isParamTrue("use_awake_db_for_icews")) {		
		...
		for (DatabaseConnectionMap::iterator i = dbs.begin(); i != dbs.end(); ++i) {
			for (DatabaseConnection::RowIterator row = i->second->iter(query); row!=i->second->end(); ++row)
				_id = ICEWSEventTypeId(row.getCellAsInt32(0), i->first);
		}
		if (_id.isNull()) {
...
void ICEWSEventType::loadEventTypes() {
	...
	*/
	if (ParamReader::hasParam("icews_event_types")) {
		ICEWSEventType::readEventTypesFromFile();
	} else if (ParamReader::hasParam("icews_db")) {
		ICEWSEventType::readEventTypesFromDatabase();
	} else {
		throw UnexpectedInputException("ICEWSEventMentionFinder::ICEWSEventMentionFinder",
