...
#ifndef D_T_TRIGRAM_STRING_FEATURE_H
#define D_T_TRIGRAM_STRING_FEATURE_H
...
/** DTWordFeature is a feature used to store an n-gram of words. Each "word" can
  ...
  *
  * DTTrigramStringFeature is a spin off from DTQuadgramFeature. See explanation
  * in DTQuintgramStringFeature.h.
  */

class DTTrigramStringFeature : public DTFeature {
public:
	DTTrigramStringFeature(const DTFeatureType *type, Symbol s1, Symbol s2, wstring &str1,
		Symbol s3)
		: DTFeature(type), _symbol1(s1), _symbol2(s2), _string1(str1), _symbol3(s3)
	{
		if (s1.is_null() || s2.is_null() || s3.is_null()) {
			throw InternalInconsistencyException(
				"DTQuadgramFeature::DTTrigramStringFeature()",
				"Attempt to create DTTrigramStringFeature with null Symbol");
		}
	}
	...
	bool operator==(const DTFeature &other) const {
		if (getFeatureType() != other.getFeatureType())
			return false;
		const DTTrigramStringFeature *otherFeature =
			static_cast<const DTTrigramStringFeature*>(&other);
	...
	size_t getHashCode() const {
		...
		return featureName.hash_code() ^
			   ...
			   ^ Symbol::hash_str(_string1.c_str());
	}
	...
	void write(UTF8OutputStream &out) const {
		...
		out << _string1.c_str() << L" " << _symbol3.to_string();
	}
...
private:
	...
	static DTTrigramStringFeature *_freeList;
#endif
};
