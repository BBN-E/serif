...
#ifndef D_T_QUADGRAM_FEATURE_H
#define D_T_QUADGRAM_FEATURE_H
...
/** DTWordFeature is a feature used to store a 4-gram of words. Each "word" can
...
class DTQuadgramFeature : public DTFeature {
public:
	DTQuadgramFeature(const DTFeatureType *type, Symbol s1, Symbol s2, Symbol s3,
		Symbol s4)
		: DTFeature(type), _symbol1(s1), _symbol2(s2), _symbol3(s3), _symbol4(s4)
	{
		if (s1.is_null() || s2.is_null() || s3.is_null() || s4.is_null()) {
			throw InternalInconsistencyException(
				"DTQuadgramFeature::DTQuadgramFeature()",
				"Attempt to create DTQuadgramFeature with null Symbol");
		}
	}
	...
	bool operator==(const DTFeature &other) const {
		if (getFeatureType() != other.getFeatureType())
			return false;
		const DTQuadgramFeature *otherFeature =
			static_cast<const DTQuadgramFeature*>(&other);
	...
	void write(UTF8OutputStream &out) const {
		...
		out << _symbol3.to_string() << L" " << _symbol4.to_string();
	}
...
private:
	...
	static DTQuadgramFeature *_freeList;
#endif
};
