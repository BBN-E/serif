...
#ifndef D_T_SEPTGRAM_FEATURE_INT_H
#define D_T_SEPTGRAM_FEATURE_INT_H
...
class DTSeptgramIntFeature : public DTFeature {
public:
	DTSeptgramIntFeature(const DTFeatureType *type, Symbol s1, Symbol s2, Symbol s3,
		Symbol s4, Symbol s5, Symbol s6, Symbol s7, int integer)
		: DTFeature(type), _symbol1(s1), _symbol2(s2), _symbol3(s3), 
		_symbol4(s4), _symbol5(s5), _symbol6(s6), _symbol7(s7), _integer(integer)
	{
		if (s1.is_null() || s2.is_null() || s3.is_null() 
			|| s4.is_null() || s5.is_null() || s6.is_null() || s7.is_null()) {
			throw InternalInconsistencyException(
				"DTSeptgramIntFeature::DTSeptgramIntFeature()",
				"Attempt to create DTSeptgramIntFeature with null Symbol");
		}
	}
	...
	virtual bool operator==(const DTFeature &other) const {
		if (getFeatureType() != other.getFeatureType())
			return false;
		const DTSeptgramIntFeature *otherFeature =
			static_cast<const DTSeptgramIntFeature*>(&other);
	...
	virtual size_t getHashCode() const {
		...
		return featureName.hash_code() ^
			   _symbol1.hash_code() ^ _symbol2.hash_code() ^ _symbol3.hash_code()
				...
				^ _symbol6.hash_code() ^ _symbol7.hash_code() 
				^ ((unsigned) _integer << 3);
...
private:
	...
	static DTSeptgramIntFeature *_freeList;
#endif
};
