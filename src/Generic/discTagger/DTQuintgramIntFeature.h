...
#ifndef D_T_QUINTGRAM_FEATURE_INT_H
#define D_T_QUINTGRAM_FEATURE_INT_H
...
class DTQuintgramIntFeature : public DTFeature {
public:
	DTQuintgramIntFeature(const DTFeatureType *type, Symbol s1, Symbol s2, Symbol s3,
		Symbol s4, Symbol s5, int integer)
		: DTFeature(type), _symbol1(s1), _symbol2(s2), _symbol3(s3), 
		_symbol4(s4), _symbol5(s5), _integer(integer)
	{
		if (s1.is_null() || s2.is_null() || s3.is_null() || s4.is_null() || s5.is_null()) {
			throw InternalInconsistencyException(
				"DTQuintgramIntFeature::DTQuintgramIntFeature()",
				"Attempt to create DTQuintgramIntFeature with null Symbol");
		}
	}
	...
	bool operator==(const DTFeature &other) const {
		if (getFeatureType() != other.getFeatureType())
			return false;
		const DTQuintgramIntFeature *otherFeature =
			static_cast<const DTQuintgramIntFeature*>(&other);
	...
	size_t getHashCode() const {
		...
		return featureName.hash_code() ^
			   _symbol1.hash_code() ^ _symbol2.hash_code() ^ _symbol3.hash_code()
			    ^ _symbol4.hash_code() ^ _symbol5.hash_code() ^ ((unsigned) _integer << 3);
	...
	void write(UTF8OutputStream &out) const {
		...
		out << _symbol5.to_string() << L" " << _integer;
	}
...
private:
	...
	static DTQuintgramIntFeature *_freeList;
#endif
};
