...
#ifndef D_T_TRIGRAM_2_INT_FEATURE_H
#define D_T_TRIGRAM_2_INT_FEATURE_H
...
/** DTTrigram2IntFeature is a feature used to store a trigram of words plus two integers. 
...
class DTTrigram2IntFeature : public DTFeature {
public:
	DTTrigram2IntFeature(const DTFeatureType *type, Symbol s1, Symbol s2, Symbol s3, int integer1, int integer2)
		: DTFeature(type), _symbol1(s1), _symbol2(s2), _symbol3(s3), _int1(integer1), _int2(integer2)
	{
		if (s1.is_null() || s2.is_null() || s3.is_null()) {
			throw InternalInconsistencyException(
				"DTTrigramFeature::DTTrigram2IntFeature()",
				"Attempt to create DTTrigram2IntFeature with null Symbol");
		}
	}
	...
	bool operator==(const DTFeature &other) const {
		if (getFeatureType() != other.getFeatureType())
			return false;
		const DTTrigram2IntFeature *otherFeature =
			static_cast<const DTTrigram2IntFeature*>(&other);
...
private:
	...
	static DTTrigram2IntFeature *_freeList;
#endif
};
