...
#ifndef D_T_QUINTGRAM_FEATURE_H
#define D_T_QUINTGRAM_FEATURE_H
...
class DTQuintgramFeature : public DTFeature {
public:
	DTQuintgramFeature(const DTFeatureType *type, Symbol s1, Symbol s2, Symbol s3,
		Symbol s4, Symbol s5)
		: DTFeature(type), _symbol1(s1), _symbol2(s2), _symbol3(s3), _symbol4(s4), _symbol5(s5)
	{
		if (s1.is_null() || s2.is_null() || s3.is_null() || s4.is_null() || s5.is_null()) {
			throw InternalInconsistencyException(
				"DTQuintgramFeature::DTQuintgramFeature()",
				"Attempt to create DTQuintgramFeature with null Symbol");
		}
	}
	...
	bool operator==(const DTFeature &other) const {
		if (getFeatureType() != other.getFeatureType())
			return false;
		const DTQuintgramFeature *otherFeature =
			static_cast<const DTQuintgramFeature*>(&other);
...
private:
	...
	static DTQuintgramFeature *_freeList;
#endif
};
