...
#ifndef HEADWORD_BIGRAM_WC_16_FT_H
#define HEADWORD_BIGRAM_WC_16_FT_H
...
class HeadwordBigramWC16FT : public P1DescFeatureType {
public:
	HeadwordBigramWC16FT() : P1DescFeatureType(Symbol(L"headword-bigram-wc-16")) {}
								...
								DTFeature **resultArray) const
	{
		...
		if(isClusterableWord(hw)){
			...
			if(isClusterableWord(prevword)){
				...
				resultArray[nfeatures++] = _new DT3IntFeature(this, state.getTag(), -1, prevWC.c16(), hwWC.c16());
			}
			if(isClusterableWord(nextword)){
				const WordClusterClass nextWC = WordClusterClass(SymbolUtilities::stemDescriptor(nextword), true);
				resultArray[nfeatures++] = _new DT3IntFeature(this, state.getTag(), 1, hwWC.c16(), nextWC.c16());
			}
		}
