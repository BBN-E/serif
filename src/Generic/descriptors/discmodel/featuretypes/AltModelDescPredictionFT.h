...
#ifndef ALTMODEL_DESC_FT_H
#define ALTMODEL_DESC_FT_H
...
class AltModelDescPredictionFT : public P1DescFeatureType {
public:
	AltModelDescPredictionFT() : P1DescFeatureType(Symbol(L"alt-model")) {}
			...
			if (prediction != Symbol(L"NONE")) { //ignore nones, since they will be too diverse
				...
				if (nfeat >= DTFeatureType::MAX_FEATURES_PER_EXTRACTION) {
					SessionLogger::warn("DT_feature_limit") 
						<<"AltModelDescPredictionFT discarding features beyond MAX_FEATURES_PER_EXTRACTION";
					break;
				}else{
