...
#ifndef HEADWORD_BIGRAM_WC_12_FT_H
#define HEADWORD_BIGRAM_WC_12_FT_H
...
class HeadwordBigramWC12FT : public P1DescFeatureType {
public:
	HeadwordBigramWC12FT() : P1DescFeatureType(Symbol(L"headword-bigram-wc-12")) {}
								...
								DTFeature **resultArray) const
	{
		...
		if(isClusterableWord(hw)){
			...
			if(isClusterableWord(prevword)){
				...
				resultArray[nfeatures++] = _new DT3IntFeature(this, state.getTag(), -1, prevWC.c12(), hwWC.c12());
			}
			if(isClusterableWord(nextword)){
				const WordClusterClass nextWC = WordClusterClass(SymbolUtilities::stemDescriptor(nextword), true);
				resultArray[nfeatures++] = _new DT3IntFeature(this, state.getTag(), 1, hwWC.c12(), nextWC.c12());
			}
		}
