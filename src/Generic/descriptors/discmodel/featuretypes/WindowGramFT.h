...
#ifndef WINDOWGRAM_FT_H
#define WINDOWGRAM_FT_H
...
class WindowGramFT : public P1DescFeatureType {
private:
...
public:
	...
	WindowGramFT() : P1DescFeatureType(Symbol(L"windowgram")) {}
								...
								DTFeature **resultArray) const
	{
		...
		int  nresults = 0;
		Symbol thisword = o->getNode()->getHeadWord();
			...
			if(!_stopWords->lookup(oth)){
				if( nresults >= DTFeatureType::MAX_FEATURES_PER_EXTRACTION){
					SessionLogger::warn("DT_feature_limit") 
						<<"WindowGramFT discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
					break;
				}else{
					resultArray[nresults++] = _new DTTrigramFeature(this, 
						state.getTag(), thisword, oth);
				}
			}
		}
		return nresults;
