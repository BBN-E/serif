...
#ifndef HEADWORD_BIGRAM_WC_8_FT_H
#define HEADWORD_BIGRAM_WC_8_FT_H
...
class HeadwordBigramWC8FT : public P1DescFeatureType {
public:
	HeadwordBigramWC8FT() : P1DescFeatureType(Symbol(L"headword-bigram-wc-8")) {}
								...
								DTFeature **resultArray) const
	{
		...
		if(isClusterableWord(hw)){
			...
			if(isClusterableWord(prevword)){
				...
				resultArray[nfeatures++] = _new DT3IntFeature(this, state.getTag(), -1, prevWC.c8(), hwWC.c8());
			}
			if(isClusterableWord(nextword)){
				const WordClusterClass nextWC = WordClusterClass(SymbolUtilities::stemDescriptor(nextword), true);
				resultArray[nfeatures++] = _new DT3IntFeature(this, state.getTag(), 1, hwWC.c8(), nextWC.c8());
			}
		}
