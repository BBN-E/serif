...
Symbol SpanishPronounLinkerUtils::augmentIfPOS(Symbol type, const SynNode *node) {
...
Symbol SpanishPronounLinkerUtils::combineSymbols(Symbol symArray[], int nSymArray, bool use_delimiter) {
...
Symbol SpanishPronounLinkerUtils::getNormDistSymbol(int distance) {
...
Symbol SpanishPronounLinkerUtils::getAugmentedParentHeadWord(const SynNode *node) {
	...
	// VC = copula (from spanish)
	/*
	if(parheadtag == SpanishSTags::VC) {
...
Symbol SpanishPronounLinkerUtils::getAugmentedParentHeadTag(const SynNode *node) {
	if(node == NULL || node->getParent() == NULL)
		return SymbolConstants::nullSymbol;
