...
SpanishRuleDescLinker::SpanishRuleDescLinker() : _debug(Symbol(L"desc_link_debug")) { }
int SpanishRuleDescLinker::linkMention (LexEntitySet * currSolution, MentionUID currMentionUID, 
...
EntityGuess* SpanishRuleDescLinker::_headNPMatch(Mention* ment, LexEntitySet *ents, EntityType linkType)
{
...
EntityGuess* SpanishRuleDescLinker::_guessNewEntity(Mention *ment, EntityType linkType)
{
	// MEMORY: linkMention, the caller of this method, is guaranteed to delete this
	EntityGuess* guess = _new EntityGuess();
...
const SynNode* SpanishRuleDescLinker::_getNumericPremod(const SynNode* node) const
{
	...
	for (int i = 0; i < headIndex; i++) {
		...
		if (pre->getTag() == SpanishSTags::QP) {
			const SynNode* head = pre->getHead();
			Symbol headTag = head->getTag();
			if (headTag == SpanishSTags::POS_Z)
				return head;
		}
		*/
	}
...
bool SpanishRuleDescLinker::_hasPremodName(Mention* ment, EntitySet* ents)
{
...
int SpanishRuleDescLinker::_getPremodNames(Mention* ment, EntitySet* ents,
	...
	for (int i = 0; i < hIndex; i++) {
		...
		for (int j = 0; j < n_names; j++) {
			if (n_results == max_results) {
				SessionLogger::logger->reportWarning() << "SpanishRuleDescLinker::_getPremodNames(): "
...
int SpanishRuleDescLinker::_getMentionInternalNames(const SynNode* node, 
										 const MentionSet* ms,
										 const SynNode* results[], 
										 const int max_results) 
{
	...
	if (ment->mentionType == Mention::NAME) {
		...
		for (int i = 0; i < node->getNChildren(); i++) {
			...
			for (int j = 0; j < n_names; j++) {
				if (n_results == max_results) {
					SessionLogger::logger->reportWarning() << "SpanishRuleDescLinker::_getMentionInternalNames: "
...
bool SpanishRuleDescLinker::_hasPremodNameEntityClash(Mention* ment, Entity* ent, EntitySet* ents) 
{
...
bool SpanishRuleDescLinker::_hasNumericClash(const SynNode* node, Entity* ent, LexEntitySet *ents) 
{
	if (ent == 0 || node == 0) {
		return false;
	}
...
int SpanishRuleDescLinker::_getHeadNPWords(const SynNode* node, Symbol results[], int max_results) {
...
bool SpanishRuleDescLinker::_hasNonDPPremod(const SynNode* node) {
	for (int i = 0; i < node->getHeadIndex(); i++) {
		if (!node->getChild(i)->getTag().isInSymbolGroup(SpanishSTags::POS_D_GROUP))
			return true;
	}
	return false;
}
