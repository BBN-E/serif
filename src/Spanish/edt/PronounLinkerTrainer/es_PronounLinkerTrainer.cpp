...
#include "Spanish/edt/PronounLinkerTrainer/es_PronounLinkerTrainer.h"
...
SpanishPronounLinkerTrainer::SpanishPronounLinkerTrainer() : _antPriorWriter(2,100), _hobbsDistWriter(2,25),
	_pronHeadWordWriter(4,250), _pronParWordWriter(2,500)
{
	...
	if (!ParamReader::getParam("pronlink_model",prefix,									 500))	{
		throw UnexpectedInputException("SpanishPronounLinkerTrainer::SpanishPronounLinkerTrainer()",
...
SpanishPronounLinkerTrainer::~SpanishPronounLinkerTrainer() {
	closeInputFile();
	closeOutputFiles();
}

void SpanishPronounLinkerTrainer::openInputFile(char *trainingFile) {
	if(trainingFile != NULL)
		_trainingSet.openFile(trainingFile);
}

void SpanishPronounLinkerTrainer::closeInputFile() {
	_trainingSet.closeFile();
}

void SpanishPronounLinkerTrainer::openOutputFiles(const char* model_prefix)
{
	char file[501];
...
void SpanishPronounLinkerTrainer::closeOutputFiles() {
...
void SpanishPronounLinkerTrainer::trainModels() {
...
void SpanishPronounLinkerTrainer::trainDocument(CorefDocument *document) {
			...
			if(!WordConstants::isLinkingPronoun(mention->getHead()->getHeadWord()))
				...
				for(j=0; document->parses[j]->getRoot() != mentionRoot; j++) {
					if(j > document->parses.length())
						throw InternalInconsistencyException("SpanishPronounLinkerTrainer::trainDocument()", "Pronoun's parse tree not found in parses array!");
				...
				if(antecedent == NULL)
					throw InternalInconsistencyException("SpanishPronounLinkerTrainer::trainDocument()", "Expected antecedent not found.");
...
void SpanishPronounLinkerTrainer::registerPronounLink(Mention *antecedent, const SynNode *antecedentNode,
...
void SpanishPronounLinkerTrainer::writeModels() {
