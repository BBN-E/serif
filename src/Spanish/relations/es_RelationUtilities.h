#ifndef es_RELATION_UTILITIES_H
#define es_RELATION_UTILITIES_H
...
class SpanishRelationUtilities : public RelationUtilities {
private:
	friend class SpanishRelationUtilitiesFactory;
public:
...
private:
	...
	static Symbol Ges_AFF_CITIZes_RESIDENT_RELIGION_ETHNICITY;
	static Symbol Ges_AFF_ORG_LOCATION;
	static Symbol PER_SOC_SUBORDINATE;

};

// RelationModel factory
class SpanishRelationUtilitiesFactory: public RelationUtilities::Factory {

	SpanishRelationUtilities* utils;
	virtual RelationUtilities *get() { 
		if (utils == 0) {
			utils = _new SpanishRelationUtilities(); 
		} 
		return utils;
	}

public:
	SpanishRelationUtilitiesFactory() { utils = 0;}
