...
UTF8OutputStream SpanishFamilyRelationFinder::_debugStream;
bool SpanishFamilyRelationFinder::DEBUG = false;


SpanishFamilyRelationFinder::SpanishFamilyRelationFinder()
{
...
void SpanishFamilyRelationFinder::resetForNewSentence()
{
}

RelMention* SpanishFamilyRelationFinder::getFixedFamilyRelation(RelMention *rm, DocTheory *docTheory) {
...
bool SpanishFamilyRelationFinder::findFamilyRelation(FamilyRelation &fr, Symbol name)
{
...
void SpanishFamilyRelationFinder::changeRelationGender(FamilyRelation &fr, std::string argGender)
{
	if (argGender == "male" && fr.gender != "male") {
		...
		if (!found){
			throw UnrecoverableException("SpanishFamilyRelationFinder::changeRelationGender()", "Could not find FamilyRelation for male version of original");
		}
	}
		...
		if (!found){
			throw UnrecoverableException("SpanishFamilyRelationFinder::changeRelationGender()", "Could not find FamilyRelation for female version of original");
		}
	}
}

std::string SpanishFamilyRelationFinder::getArgGender(Symbol *mheads, int beginIndex, int endIndex){
	int maleCount(0);
	int femaleCount(0);
...
void SpanishFamilyRelationFinder::getMostCommonRelation(FamilyRelation &answer, std::vector<Symbol> *frlist)
{
...
void SpanishFamilyRelationFinder::fixSpouseRelation(FamilyRelation &fr, std::string gender)
{
...
Symbol SpanishFamilyRelationFinder::getMentionHeadFromEntity(Entity *ent, int entityIndex, DocTheory *dt){
...
void SpanishFamilyRelationFinder::fillGenderEvidenceArrays(Sexp* sexp) {
	if (!sexp->isList() || sexp->getNumChildren() < 3 || !sexp->getFirstChild()->isAtom())
		throw UnexpectedInputException("SpanishFamilyRelationFinder::fillGenderEvidenceArrays()", 
...
void SpanishFamilyRelationFinder::fillRelationArrays(Sexp* sexp, int i)
{
	if (!sexp->isList() || sexp->getNumChildren() < 7 || !sexp->getFirstChild()->isAtom())
		throw UnexpectedInputException("SpanishFamilyRelationFinder::fillRelationArrays()", 
				"Ill-formed values-rule-set sexp");
...
void SpanishFamilyRelationFinder::printFamilyRelation(FamilyRelation fr)
{
