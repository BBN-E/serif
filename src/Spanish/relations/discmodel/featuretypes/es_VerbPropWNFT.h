...
#ifndef es_VERB_PROP_WN_FT_H
#define es_VERB_PROP_WN_FT_H
...
class SpanishVerbPropWNFT : public P1RelationFeatureType {
public:
	SpanishVerbPropWNFT() : P1RelationFeatureType(Symbol(L"verb-prop-wordnet")) {}
		...
		if (o->getVerbinProp() != Symbol(L"NULL") ) {
			...
			while (level >= 0) {
				if (nfeatures >= DTFeatureType::MAX_FEATURES_PER_EXTRACTION){
						SessionLogger::warn("vpwnft_disc_0") <<"es_VerbPropWNFT discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
						break;
				}
