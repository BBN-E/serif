...
#ifndef es_PP_REL_WN_FT_H
#define es_PP_REL_WN_FT_H
...
class SpanishPPRelWNFT : public P1RelationFeatureType {
public:
	SpanishPPRelWNFT() : P1RelationFeatureType(Symbol(L"pp-relation-wordnet")) {}
			...
			if (o->getVerbinPPRel() != Symbol(L"NULL")) {
				...
				while (level >= 0) {
					if (nfeatures >= DTFeatureType::MAX_FEATURES_PER_EXTRACTION){
						SessionLogger::warn("prwnft_disc_0") <<"es_PPRelWNFT discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
						break;
					}
				...
				while (level >= 0) {
					if (nfeatures >= DTFeatureType::MAX_FEATURES_PER_EXTRACTION){
						SessionLogger::warn("eprw_disc_0") 
							<<"es_PPRelWNFT discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
						break;
					}
					resultArray[nfeatures++] = _new DTQuintgramIntFeature(this, state.getTag(),
