...
#ifndef es_SIMPLE_PP_REL_WN_FT_H
#define es_SIMPLE_PP_REL_WN_FT_H
...
class SpanishSimplePPRelWNFT : public P1RelationFeatureType {
public:
	SpanishSimplePPRelWNFT() : P1RelationFeatureType(Symbol(L"simple-pp-relation-wordnet")) {}
			...
			if (o->getVerbinPPRel() != Symbol(L"NULL")) {
				...
				while (level >= 0) {
					if (nfeatures >= DTFeatureType::MAX_FEATURES_PER_EXTRACTION){
						SessionLogger::warn("sprwnft_disc_0") <<"es_SimplePPRelWNFT discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
						break;
					}
				...
				while (level >= 0) {
					if (nfeatures >= DTFeatureType::MAX_FEATURES_PER_EXTRACTION){
						SessionLogger::warn("eprw_disc_1") 
							<<"es_SimplePPRelWNFT discarding features beyond MAX_FEATURES_PER_EXTRACTION\n";
						break;
					}
					resultArray[nfeatures++] = _new DTTrigramIntFeature(this, state.getTag(),
