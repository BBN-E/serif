...
#ifndef ES_PRONOUN_CLASSIFIER_H
#define ES_PRONOUN_CLASSIFIER_H
 ...
 * 
 */
class SpanishPronounClassifier : public PronounClassifier {
private:
	friend class SpanishPronounClassifierFactory;

public:

	~SpanishPronounClassifier();
...
private:
	SpanishPronounClassifier();
	bool hasDroppedPronounParent(const SynNode* node);

};

class SpanishPronounClassifierFactory: public PronounClassifier::Factory {
	virtual PronounClassifier *build() { return _new SpanishPronounClassifier(); }
};
