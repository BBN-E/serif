...
Span* loadIcewsSentenceSpan(SerifXML::XMLTheoryElement e) { 
	Span *span = new IcewsSentenceSpan(); 
	span->loadXML(e); 
	return span; 
} 
...
int IcewsSentenceSpan::edtOffsetToIcewsSentenceNo(EDTOffset offset, const DocTheory* docTheory) {
	if (ParamReader::getRequiredTrueFalseParam("use_icews_sentence_numbers")) {
		...
		SessionLogger::err("ICEWS") << "Unable to find ICEWS sentence number for EDT offset "
		...
		SessionLogger::err("ICEWS") << "Unable to find sentence number for EDT offset "
			<< offset.value() << "; returning 0.";
		return 0;
	}
}
