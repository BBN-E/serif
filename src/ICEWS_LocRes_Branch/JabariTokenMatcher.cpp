	...
	void matchTokens(const TokenSequence *toks, const Symbol *posTags, size_t start_toknum, size_t current_toknum, std::vector<Match> &result, MatchType matchType) {
		...
		if (_matchInfo) {
			BOOST_FOREACH(MatchIds match_ids, _matchInfo->match_ids) {
				addMatch(match_ids, start_toknum, current_toknum-1, _matchInfo->pattern_strlen, result, matchType);
			}
		}
...
namespace { // Helper methods declarations.
	template<typename ValueIdType> ValueIdType makeValueId(const char* value, Symbol);
...
template<typename ValueIdType, typename PatternIdType>
JabariTokenMatcher<ValueIdType, PatternIdType>::JabariTokenMatcher(const char* kind, bool read_patterns): _kind(kind), _size(0) {
	Trie::stripPunctuation(ParamReader::isParamTrue("icews_strip_punctuation_when_matching_jabari_patterns"));
	...
	// Read patterns from the database.
	if (!boost::iequals(_kind, "composite_actor")) {
		...
		for (DatabaseConnection::RowIterator result = icews_db->iter(query); result!=icews_db->end(); ++result) {
			PatternIdType patternId(result.getCellAsInt32(0));
			Symbol code(result.getCellAsSymbol(2));
			ValueIdType valueId = makeValueId<ValueIdType>(result.getCell(1), code);
...
namespace { // Helper methods definitions.
	...
	ValueIdType makeValueId(const char* value, Symbol) {
		return value ? ValueIdType(boost::lexical_cast<typename ValueIdType::id_type>(value)) : ValueIdType();
	}
	...
	ValueIdType getIdForCode(Symbol code, const std::string &kind) {
		...
		DatabaseConnection_ptr icews_db(ICEWS::getSingletonIcewsDb());
		for (DatabaseConnection::RowIterator result = icews_db->iter(valueIdQuery); result!=icews_db->end(); ++result)
			return makeValueId<ValueIdType>(result.getCell(0), code);
	...
	template<>
	CompositeActorId makeValueId<CompositeActorId>(const char* value, Symbol code) {
