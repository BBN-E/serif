...
public:	
	...
	BOOST_MAKE_SHARED_5ARG_CONSTRUCTOR(ICEWSEventType, ICEWS::ICEWSEventTypeId, Symbol, std::wstring, std::wstring, const std::vector<Symbol>&);
	...
	/** Return true if it's possible for the two specified roles to be filled
	  * by the same entities.  E.g., if role1 and role2 are SOURCE and TARGET,
	  * then this indicates whether the event can be reflexive. */
	bool rolesCanBeFilledByTheSameEntity(Symbol role1, Symbol role2) const;

	/*=========================== Registry ============================ */
	static boost::shared_ptr<ICEWSEventType> getEventTypeForCode(Symbol code);

	/** Register a set of event types, based on parameters in the parameter
	  * file.  If the icews_event_types parameter is set, then the set of
	  * event types is loaded from the file with the specified filename.
	  * Otherwise, if the icews_mysql_* parameters are set, then the set of
	  * event types is loaded from the mysql database.  An error will be
	  * reported if both parameters are set. */
	static void loadEventTypes();

	/** Manually register an event type.  See also loadEventTypes(), which
	  * registers event types based on parameters in the param file. */
