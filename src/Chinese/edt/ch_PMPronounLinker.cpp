...
DebugStream &ChinesePMPronounLinker::_debugStream = ChinesePronounLinker::getDebugStream();

ChinesePMPronounLinker::ChinesePMPronounLinker() {
...
void ChinesePMPronounLinker::addPreviousParse(const Parse *parse) {
	_previousParses.push_back(parse);
}

void ChinesePMPronounLinker::resetPreviousParses() {
	_previousParses.clear();
}

int ChinesePMPronounLinker::linkMention (LexEntitySet * currSolution,
...
int ChinesePMPronounLinker::getLinkGuesses(EntitySet *currSolution,
								  Mention *currMention,
								  LinkGuess results[],
								  int max_results)
{
...
bool ChinesePMPronounLinker::_isLinkedBadly(EntitySet *currSolution, Mention* pronMention,
								     Entity* linkedEntity)
{
	const SynNode *pronNode = pronMention->getNode();

	// "it" shouldn't be linked to person
	if ((pronNode->getHeadWord() == ChineseWordConstants::IT ||
		 pronNode->getHeadWord() == ChineseWordConstants::ITS) &&
...
char *ChinesePMPronounLinker::debugTerminals(const SynNode *node) {
...
void ChinesePMPronounLinker::_initialize(const char* model_prefix)
{
...
void ChinesePMPronounLinker::verifyEntityTypes(const char *file_name) {
	...
	if (priorAntStream.fail()) {
		throw UnexpectedInputException(
			"ChinesePronounLinker::verifyEntityTypes()",
...
bool ChinesePMPronounLinker::_isSpeakerMention(Mention *ment) { 
		return (_docTheory->isSpeakerSentence(ment->getSentenceNumber()));
}
	
bool ChinesePMPronounLinker::_isSpeakerEntity(Entity *ent, EntitySet *ents) {
	if (ent == 0)
		return false;
