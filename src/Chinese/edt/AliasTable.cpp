...
int AliasTable::lookup(Symbol key, Symbol results[], int max_results) {
	...
	if (result != NULL) {
		...
		if (value->length > max_results) {
			SessionLogger::warn("alias_table") << "AliasTable::lookup(): Number of aliases returned exceeds max_results.";
		}
...
throw(UnexpectedInputException)
{
	...
	if(token.symValue() == SymbolConstants::leftParen) {
		...
		if (token.symValue() != SymbolConstants::rightParen) {
			SessionLogger::warn("alias_table") << "AliasTable::getAliasSymbolArray(): Number of aliases read exceeds max_results.";
		}
	}
