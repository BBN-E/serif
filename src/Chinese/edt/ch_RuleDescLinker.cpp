...
ChineseRuleDescLinker::ChineseRuleDescLinker() : _debug(Symbol(L"desc_link_debug")) { }
int ChineseRuleDescLinker::linkMention (LexEntitySet * currSolution, MentionUID currMentionUID, 
...
EntityGuess* ChineseRuleDescLinker::_headNPMatch(Mention* ment, LexEntitySet *ents, EntityType linkType)
{
	...
	const SynNode* numeric = _getNumericPremod(ment->node);
	if (numeric != 0) 
			...
			if (match) {
				...
				if (prevMent->mentionType == Mention::DESC) {
					...
					if (numeric != 0) {
						...
						const SynNode* prevNumeric = _getNumericPremod(prevMent->node);
						if (prevNumeric == 0) {
					...
					if (has_premod_name && _hasPremodNameEntityClash(ment, ent, ents)) {
...
EntityGuess* ChineseRuleDescLinker::_guessNewEntity(Mention *ment, EntityType linkType)
{
	// MEMORY: linkMention, the caller of this method, is guaranteed to delete this
	EntityGuess* guess = _new EntityGuess();
...
const SynNode* ChineseRuleDescLinker::_getNumericPremod(const SynNode* node) const
{
	...
	for (int i = 0; i < headIndex; i++) {
		...
		if (pre->getTag() == ChineseSTags::QP) {
			const SynNode* head = pre->getHead();
			Symbol headTag = head->getTag();
			if (headTag == ChineseSTags::CD || headTag == ChineseSTags::OD)
				return head;
		}
	}
	return 0;
...
bool ChineseRuleDescLinker::_hasPremodName(Mention* ment, EntitySet* ents)
{
...
int ChineseRuleDescLinker::_getPremodNames(Mention* ment, EntitySet* ents,
	...
	for (int i = 0; i < hIndex; i++) {
		...
		for (int j = 0; j < n_names; j++) {
			if (n_results == max_results) {
				SessionLogger::warn("desc_link") << "ChineseRuleDescLinker::_getPremodNames(): "
...
int ChineseRuleDescLinker::_getMentionInternalNames(const SynNode* node, 
										 const MentionSet* ms,
										 const SynNode* results[], 
										 const int max_results) 
{
	...
	if (ment->mentionType == Mention::NAME) {
		...
		for (int i = 0; i < node->getNChildren(); i++) {
			...
			for (int j = 0; j < n_names; j++) {
				if (n_results == max_results) {
					SessionLogger::warn("desc_link") << "ChineseRuleDescLinker::_getMentionInternalNames: "
...
bool ChineseRuleDescLinker::_hasPremodNameEntityClash(Mention* ment, Entity* ent, EntitySet* ents) 
{
	...
	for (int k = 0; k < ent->mentions.length(); k++) {
		...
		for (int i = 0; i < n_ment1_pnames; i++) {
			_debug << "In clash: ment1:\n" << ment1PNames[i]->toString(1) << "\n";

			Mention* m1 = ms1->getMentionByNode(ment1PNames[i]);
			...
			for (int j = 0; j < n_ment2_pnames; j++) {
				_debug << "In clash: ment2:\n" << ment2PNames[j]->toString(1) << "\n";
				
				Mention* m2 = ms2->getMentionByNode(ment2PNames[j]);
				// TODO: need to account for metonymyic mentions
...
bool ChineseRuleDescLinker::_hasNumericClash(const SynNode* node, Entity* ent, LexEntitySet *ents) 
{
	if (ent == 0 || node == 0) {
		return false;
	}
...
int ChineseRuleDescLinker::_getHeadNPWords(const SynNode* node, Symbol results[], int max_results) {
...
bool ChineseRuleDescLinker::_hasNonDPPremod(const SynNode* node) {
	for (int i = 0; i < node->getHeadIndex(); i++) {
		if (node->getChild(i)->getTag() != ChineseSTags::DP)
			return true;
	}
	return false;
}
