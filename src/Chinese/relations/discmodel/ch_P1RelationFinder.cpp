...
UTF8OutputStream ChineseP1RelationFinder::_debugStream;
bool ChineseP1RelationFinder::DEBUG = false;

ChineseP1RelationFinder::ChineseP1RelationFinder() {
	...
	std::string model_file = ParamReader::getRequiredParam("p1_relation_model_file");
	...
	_decoder = _new P1Decoder(_tagSet, featureTypes, weights, _overgen_percentage);
...
void ChineseP1RelationFinder::cleanup() {
}


void ChineseP1RelationFinder::resetForNewSentence() {
	_n_relations = 0;
}

RelMentionSet *ChineseP1RelationFinder::getRelationTheory(EntitySet *entitySet,
												   const Parse *parse,
	...
	for (int i = 0; i < nmentions; i++) {
		...
		for (int j = i + 1; j < nmentions; j++) {
			...
			if (_decoder->DEBUG) {
				...
				if (discAnswer != vectorAnswer &&
					vectorAnswer != _tagSet->getNoneTag())
				{
					_decoder->_debugStream << L"V says yes; P1 says no\n";
				}

			}

			Symbol answer = discAnswer;
			/*if (discAnswer == _tagSet->getNoneTag() &&
				vectorAnswer != _tagSet->getNoneTag())
			{
				answer = vectorAnswer;
			}*/
...
void ChineseP1RelationFinder::addRelation(const Mention *first, const Mention *second, Symbol type) {
