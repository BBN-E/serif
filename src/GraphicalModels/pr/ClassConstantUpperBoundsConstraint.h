#ifndef _CLASS_CONSTANT_BOUND_CONSTRAINT_H_
#define _CLASS_CONSTANT_BOUND_CONSTRAINT_H_
...
class ClassConstantUpperBoundConstraint : public GraphicalModel::Constraint<typename View::GraphType>{
public:
	typedef std::vector<unsigned int> Classes;

	ClassConstantUpperBoundConstraint(const typename View::GraphType& graph, 
			...
			for (size_t fact = 0; fact < _view.nFactors(graph); ++fact) {
				...
				for (typename Classes::const_iterator cIt = _classes.begin(); cIt!=_classes.end(); ++cIt) {
					factor.adjustModifier(*cIt, -this->_weight, 
							const_cast<ClassConstantUpperBoundConstraint*>(this));
				}
			}
		}
	}
			...
			for (size_t fact = 0; fact < _view.nFactors(graph); ++fact) {
				...
				for (typename Classes::const_iterator cIt = _classes.begin(); cIt!=_classes.end(); ++cIt) {
					if (!got_anything) {
						...
						out << L"Expectation: " << this->_expectation 
							<< L"; Bounds: " << _constant_bound << L"\n";
					}
					out << L"\t\tClass " << *cIt << L" for variable " 
	...
	double gradient() const {
		return -this->_bound + this->_expectation;
	}
