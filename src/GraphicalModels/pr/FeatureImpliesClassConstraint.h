#ifndef _FEATURE_IMPLIES_CLASS_CONSTRAINT_H_
#define _FEATURE_IMPLIES_CLASS_CONSTRAINT_H_
...
class FeatureImpliesClassConstraint : public GraphicalModel::Constraint<typename View::GraphType> 
{
public:
	...
	FeatureImpliesClassConstraint(const Data& graphs, const Strings& features,
			const Classes& classes, double threshold,
			const View& adaptor) 
		: GraphicalModel::Constraint<typename View::GraphType>(L"<="), _classes(classes), 
		_cachedBounds(graphs.nGraphs()), _marked(graphs.nFactors()), _view(adaptor),
		_resolvedFeatures()
	{
			...
			if (_marked[factor.id]) {
				for (typename Classes::const_iterator cIt = _classes.begin(); cIt!=_classes.end(); ++cIt) {
					factor.adjustModifier(*cIt, this->_weight, 
							const_cast<FeatureImpliesClassConstraint*>(this));
					//factor.modifiers()[*cIt] += this->_weight;
				}
			}
		}
	}
		...
		for (size_t fact = 0; fact < _view.nFactors(g); ++fact) {
			...
			if (_marked[factor.id]) {
				for (typename Classes::const_iterator cIt = _classes.begin(); cIt!=_classes.end(); ++cIt) {
					...
					out << L"\t\tClass " << *cIt << L" of variable " << fact
	...
	void updateBoundAndExpectation(const typename View::GraphType& graph) {
		// this needed to make compiler instantiate parent template
...
struct FICCDescription {
	std::vector<std::wstring> classes;
	std::vector<std::wstring> features;
};

FICCDescription parseFICCDescription(const std::wstring& constraintData); 
};
