...
class TempUIHook : public PostIterationCallback {
public:
	TempUIHook(TemporalTrainerUI* t) : _t(t) {}
	virtual bool operator()(); 
private:
	TemporalTrainerUI* _t;
};

bool TempUIHook::operator ()() {
	return _t->updateBetweenIterations();
}

TemporalTrainerUI::TemporalTrainerUI(TemporalTrainer_ptr trainer, 
									 ...
									 StringStore_ptr preview_strings) 
: _trainer(trainer), _alData(data), _strategy(strategy),
_finished(false), _instanceStrategy(instanceStrategy),
_preview_strings(preview_strings), _instanceHashes(instanceHashes)
{
	trainer->registerPostIterationHook(make_shared<TempUIHook>(this));
	populateFeatureGrid();
}

bool TemporalTrainerUI::updateBetweenIterations() {
	_strategy->modelUpdate();
	return _alData->updateBetweenIterations();
}

void TemporalTrainerUI::populateFeatureGrid() {
	_featureGrid.clear();
...
TemporalTrainer_ptr TemporalTrainerUI::trainer() {
	return _trainer;
}
 ...
 * AJAX event handlers
 ***************************/
const void* TemporalTrainerUI::event_handler(enum mg_event event,
...
void TemporalTrainerUI::ajax_get_penalties(struct mg_connection* conn,
...
void TemporalTrainerUI::ajax_active_learning_done(struct mg_connection* conn,
			const struct mg_request_info* request_info) 
{
	MongooseUtils::begin_ajax_reply(conn);
	stringstream json;
...
void TemporalTrainerUI::send_ajax_feature_list(
	struct mg_connection* conn, const struct mg_request_info* request_info)
{
	stringstream json;
	json << "[";
...
void TemporalTrainerUI::ajax_record_feature(struct mg_connection* conn,
	  const struct mg_request_info* request_info)
{
...
void TemporalTrainerUI::syncToDB() {
	_trainer->syncToDB();
	_alData->syncToDB();
}
...
void TemporalTrainerUI::feature_as_json(int feat, std::stringstream& json) {
	...
	json << "{ \"idx\" : " << feat << ", \"weight\" : " <<
		...
		"\"attribute\" : " << 
			JSONUtils::jsonify(JSONUtils::html_escape(
						_trainer->featureAttribute(feat))) << ", " <<
		"\"instance_data\" : ";
...
TemporalTrainerUI::clientInstances(int feat, size_t max_instances) const {
...
ClientInstance TemporalTrainerUI::makeClientInstance(int inst) const {
...
wstring TemporalTrainerUI::previewString(int inst) const {
	wstringstream ret;
	double pred = _trainer->prediction(inst);
	ret << "<font size='large' color='#999999'>" << std::setprecision(3) << pred
		<< ": </font> " <<  _preview_strings->getString(inst);

	return ret.str();
}

void TemporalTrainerUI::updateInstanceAnnotation(const string& encoded) {
...
bool TemporalTrainerUI::finished() const {
	return _finished;
}
