...
warnings.warn("serifxml.py in Active/Projects/SERIF/python is deprecated.  Please text-open/src/python/serifxml3.py instead.", DeprecationWarning)
...
class SerifMentionTheory(SerifTheory):
    ...
    def text(self):
        ...
        if self.syn_node is None: return None
        else: return self.syn_node.text

    @property
    def tokens(self):
        """The tokens contained in this mention's SynNode."""
        if self.syn_node is None: return None
        else: return self.syn_node.tokens
    ...
    def _get_summary(self):
        if self.syn_node is None: return None
        else: return self.syn_node._get_summary()
...
class SerifSynNodeTheory(SerifSequenceTheory):
    ...
    def head_tag(self):
        ...
        else: return self.head.head_tag
...
class Proposition(SerifTheory):
    ...
    def _get_summary(self):
        ...
        else: pred = self.pred_type.value
        return '%s(%s)' % (pred,
...
class EventMentionType(SerifOffsetTheory):
    score               = _SimpleAttribute(float, default=1.0)
    event_type          = _SimpleAttribute(is_required=True)

class EventMentionAnchor(SerifOffsetTheory):
    anchor_prop         = _ReferenceAttribute('anchor_prop_id',
                                              cls=Proposition)
    anchor_node         = _ReferenceAttribute('anchor_node_id',
                                              cls=SynNode)
...
def send_serifxml_pm_request(document, hostname=HOSTNAME, port=PORT,
                             ...
                             slot1_start_offset=-1, slot2_start_offset=-1, slot3_start_offset=-1, equiv_names='', verbose=False, timeout=0, num_tries=1):
    """
    ...
    @param port: The port on which the Serif HTTP server is listening.
    """
    request = PATTERN_MATCH_DOCUMENT_TEMPLATE % dict(document=document, pattern_set_name=pattern_set_name, slot1=slot1, slot2=slot2, slot3=slot3,
                                                     slot1_weights=slot1_weights, slot2_weights=slot2_weights, slot3_weights=slot3_weights,
                                                     slot1_start_offset=slot1_start_offset, slot2_start_offset=slot2_start_offset, slot3_start_offset=slot3_start_offset, equiv_names=equiv_names)
    response = send_serif_request(
        'POST SerifXMLRequest', request,
...
if __name__ == '__main__':
    ...
    if len(args) == 2 and args[0] in ('process_batch_file', 'shutdown'):
        ...
        sys.exit("Usage is %s process_batch_file|shutdown server_info_file|host:port (-i <input_batch_file> -o <output_dir>)" % sys.argv[0])
