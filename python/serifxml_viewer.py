...
def entity_dot_label1(entity,mention):
    ...
    for (m, port) in mention_texts:
        ...
        if mention is not None and mention.text==text :
             s += '<TR><TD><FONT COLOR="red">"%s(%s)%s"</FONT></TD></TR>' % (parts[0],escape_xml(m.atomic_head.text),last_part)
           # s += '<TR><TD><FONT COLOR="red">"%s" (%s)</FONT></TD></TR>' % (quote_dot1(text).replace("\l"," ").strip(),m.atomic_head.text)
        else: s += '<TR><TD>"%s(%s)%s"</TD></TR>' % (parts[0],escape_xml(m.atomic_head.text),last_part)
...
class EntityDotNode1(DotNode):
    ...
    def fill_color(self): return '#c0ffc0'
    def label(self):
        if self.small: return escape_dot(name_for_entity(self.key), wrap=True)
        else: return entity_dot_label1(self.key,self.value)
    def tooltip(self): return name_for_entity(self.key)
    def url(self):
        if not self.small: return DotNode.url(self)
...
def write_sentence_details_page(sentence, out_dir, show_merged_graphs):
    panes = [
        ('Parse', parse_to_html(sentence)),
        ('Propositions', propositions_to_html(sentence)),
        ('Relations', relations_to_html(sentence)),
        ('Events', events_to_html(sentence)),

        ]
    if show_merged_graphs:
        panes.append(('Merged Graph', merged_graph_to_html(sentence)))
...
def write_doc_entities_page(doc, out_dir):
    ...
    # Next, add a table.
    CONTEXT_SIZE = 30
    def sortkey(entity): return -len(entity.mentions)
...
def sentence_to_html(sent, highlights={}):
    """
    ...
    for mention in mentions:
        ...
        if entity is None: entity_css=''
        else: entity_css = ' entity-%s' % dot_id(entity)
...
def save(s, out_dir, filename):
    ...
    if VERBOSE: print 'Saving: %r' % path
...
def write_document_pages(doc, out_dir, options):
    ...
    if options.show_doc_graphs:
        ...
        if options.show_merged_graphs:
            write_doc_graph_page(doc, 'merged-graph', out_dir,
                                 merged_graph_to_html(doc))
...
class DocSearcher(object):
    ...
    def _add_sentence_match(self, sentence, highlighted_tokens={},
                            highlighted_synnodes={}):
...
def write_all_pages(html_files, out_dir, options):
    ...
    for index in range(len(tuples)):
        ...
        if options.show_doc_pages:
            sys.stdout.write('Processing %d of %d: %s...' % (index, len(tuples), doc.docid))
            sys.stdout.flush()
            write_document_pages(doc, out_dir, options)
            nav_items += doc_nav_to_html(doc, options.show_doc_entities, options.show_doc_graphs, options.show_merged_graphs)

        # Apply any searches.
        if searchers:
            sys.stdout.write('Searching %s...' % doc.docid)
            sys.stdout.flush()
            num_hits = 0
            for searcher in searchers:
                num_hits += searcher.update(doc)
            print ' [%d hit(s)]' % num_hits

        if n and n%5==0:
            sys.stdout.write('Writing partial index.html...')
            sys.stdout.flush()
            write_nav_page(nav_items, out_dir)
            print ' [done]'

    for searcher in searchers:
        sys.stdout.write('Writing %s...' % searcher.html_name)
        sys.stdout.flush()
    ...
    # Navigation page.
    sys.stdout.write('Writing index.html...')
    sys.stdout.flush()
    write_nav_page(nav_items, out_dir)
    print ' [done]'
...
def cli():
    ...
    if platform.system() == 'Windows':
        if not (which('dot.exe') and which('fdp.exe')):
            sys.exit("Could not find GraphViz' dot.exe and fdp.exe in the path.  Please install GraphViz and set your path accordingly.")
    else:
        if not (which('dot') and which('fdp')):
            sys.exit("Could not find GraphViz' dot and fdp in the path.  Please install Graphviz and set your path accordingly.")

    if not inputs:
        parser.error("Expected at least one input file")
    inputs = [input.decode("utf-8") for input in inputs]
    out_dir = options.out_dir.decode("utf-8")
    if not out_dir:
        parser.error("Output directory is required.")

    # Find our resource directory.
    data_dir = os.path.abspath(os.path.join(os.path.split(sys.argv[0])[0],
