...
for my $name (@names){
	...
	my @value_mentions = $value->getElementsByTagName( 'value_mention' );
	my $value_mention = $value_mentions[0];
	my $is_timex = 0;
	if (!$value_mention) {
	    $is_timex = 1;
	    @value_mentions = $value->getElementsByTagName( 'timex2_mention' );
	    $value_mention = $value_mentions[0];
	}
	...
	my @extents = $value_mention->getElementsByTagName( 'extent' );
	my $extent = $extents[0];
	my @charseqs = $extent->getElementsByTagName( 'charseq' );
	my $charseq = $charseqs[0];
	my $start     = $charseq->getAttribute( "START" );
	my $end       = $charseq->getAttribute( "END" );
...
sub new {
    ...
    foreach my $event (@eve_list) {
	push @events, $event;
    }

    my @timex_list = $DOMkey->getElementsByTagName( 'timex2' );
    foreach my $timex (@timex_list) {
	
	my @timex_mentions = $timex->getElementsByTagName( 'timex2_mention' );
	my $timex_mention = $timex_mentions[0];
	my $id = $timex_mention->getAttribute( 'ID' );
	my @extents = $timex_mention->getElementsByTagName( 'extent' );
	my $extent = $extents[0];
	my @charseqs = $extent->getElementsByTagName( 'charseq' );
	my $charseq = $charseqs[0];
	my $start = $charseq->getAttribute( 'START' );
	...
	my @anchors = $value_mention->getElementsByTagName( 'extent' );
	my $anchor = $anchors[0];
	my @charseqs = $anchor->getElementsByTagName( 'charseq' );
	my $charseq = $charseqs[0];
	my $start = $charseq->getAttribute( 'START' );
...
sub get_extent 
{
    ...
    my @mention_extents = $mention->getElementsByTagName( 'extent' );
    my $mention_extent = $mention_extents[0];
    my @mention_charseq = $mention_extent->getElementsByTagName( 'charseq' );
    my $mention_charseq = $mention_charseq[0];
    my @heads = $mention->getElementsByTagName( 'head' );
    my $head = $heads[0];
    my @head_charseqs = $head->getElementsByTagName( 'charseq' );
    my $head_charseq = $head_charseqs[0];
