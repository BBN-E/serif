...
#
# version 14
#    * bug fix in relation and event score computation to accrue penalty for
#      false alarm mentions
#    * corrected calculation of argument scores in relation/event mapping output
#
# version 13
#    * added requirement that system source type agrees with reference source type
#      for all documents being evaluated.
#    * added mention score and mention ID to entity mapping output
#    * added argument score to relation and event mapping output
#    * changed the default parameters to set the value of timex2 attributes to zero
#      and to increase the value of timex2 detection from 0.10 to 0.50.
#
# version 12
#    * bug fix in entity score computation to accrue penalty for false alarm mentions
#
# version 11a
#    * Inadvertent exclusion of values/timex2's from relation/event scoring fixed.
#      This occurred whenever system output did not include any values/timex2's.
#
# version 11
#    * Improved diagnostic error messages for RMD and VMD mapping.
#    * Elimination of unnecessary mapping and confusing mapping printout
#      of argument elements used to support RMD/VMD scoring.
#    * Improved diagnostic error messages for RDR arguments, which now
#      include flagging ROLE errors for asymmetric relations.
#
# version 10
#    * allow Arg-1/Arg-2 symmetry for ref/sys relation mapping
#      - with no penalty if there exists a mismatch in relation TYPE or SUBTYPE
#      - with TYPE and SUBTYPE penalties when relation TYPE and SUBTYPE match
#
# version 09
#    * RMD and VMD evaluations added
#    * bug fix in reading input data
#    * bug fix in relation score computation
#
# version 08
#    * default parameters changed:
#      - TYPE/SUBTYPE error weights are now 1.0/0.7 for relations
#      - relation/event mapping candidates are limited to those with mappable arguments
#    * entity LEVEL errors included in error count (has no effect on value score)
#    * Arg-* relation argument overlap requirement added (opt_N)
#
# version 07
#    * changed level-weighted entity value to use global levels (rather than
#      local document levels) and used min of ref and sys levels (rather than
#      ref level)
#    * fixed bug in calculation of relation and event value score
#    * corrected the specification of symmetric relations
#
# version 06
#    * allowed relations without mentions for relations of type METONYMY
#    * supressed the "multiple argument" warning message for relation arguments
#      with "time" type roles
#    * fixed bugs in entity_mention_score and compute_argument_map
#    * added optional check for duplicate elements in ref and tst input data
#    * made the evaluation of mention entities (no coreference) optional
#      because it is of secondary interest but can dominate CPU time
#
# version 05
#    * updated to conform to dtd version 5.1.0
#    * fixed bug in reporting of relation argument errors
#    * included reporting of mentions in relation mapping output
#    * changed event mention overlap check from anchor to extent
#    * changed command line options to improve control of defaults
#    * added ref_fa_wgt parameter for timex2s/values in analogy to entities
#
# version 04
#    * bug fix in calculation of RDR and VDR scores to include argument-level
#      penalty for arguments that are unmapped
#    * bug fix in the labeling of "entity" (= mention) LEVEL for EMD scoring
#    * provide optional EDR scoring formula to normalize out multiple mentions
#      so that entity value is a function only of entity level (per document)
#    * provide option to require that for RDR and VDR system arguments must map
#      to ref arguments at the element level in order to map at the argument level
#    * provide option to use the geometric mean of argument scores for relation
#      and event value computation
#    * provide option to require that all RDR and VDR system arguments must map
#      to all ref arguments in order to get a non-zero argument score
#
# version 03
...
use vars qw ($opt_R $opt_r $opt_T $opt_t $opt_W $opt_M $opt_N $opt_S $opt_A $opt_P);
...
my $timex2_mention_ref_fa_wgt = 0.00;
...
my $quantity_mention_ref_fa_wgt = 0.00;
...
my %parameter_set =
  (DEFAULT => {},
   "2005+TERN" => {min_text_match => 0.0,
		   timex2_attribute_wgt => {ANCHOR_DIR => 0.25,
					    ANCHOR_VAL => 0.50,
					    MOD        => 0.10,
					    SET        => 0.10,
					    TYPE       => 0.10,
					    VAL        => 1.00}},
   "2005-TERN" => {min_text_match => 0.0,
		   timex2_attribute_wgt => {ANCHOR_DIR => 0.00,
					    ANCHOR_VAL => 0.00,
					    MOD        => 0.00,
					    SET        => 0.00,
					    TYPE       => 0.50,
...
my ($allow_wrong_mapping, $geometric_averaging);
...
my $usage = "\n\n$0 -R <ref_file> -T <tst_file>\n\n".
  ...
  "  -A = \"arithmetic\" or \"geometric\".  This controls how the argument scores\n".
  "         are averaged.  The default is \"$opt_A\".\n".
...

{
  ...
  use Getopt::Std;
  getopts ('R:r:T:t:W:M:N:S:A:P:cmeasdh');
  ...
  $geometric_averaging = $opt_A =~ /geometric/i;
  $opt_A =~ /^(arithmetic|geometric)$/i or die
    "\n\nPARAMETER ERROR:  averaging (-A) was specified as '$opt_A' but must be 'arithmetic' or 'geometric'$usage";
  $all_args_required ||= $geometric_averaging;
  
  select_parameter_set ();
  not $geometric_averaging or $relation_argument_fa_wgt == 0 or die
    "\n\nPARAMETER ERROR:  relation_argument_fa_wgt must be = 0 when using geometric argument mean scoring$usage";
  not $geometric_averaging or $event_argument_fa_wgt == 0 or die
    "\n\nPARAMETER ERROR:  event_argument_fa_wgt must be = 0 when using geometric argument mean scoring$usage";
...
#evaluate relations
  if ((keys %{$ref_database{relations}})>0 and (keys %{$tst_database{relations}})>0) {
    ...
    score_releve_attribute_recognition ("relation", \@relation_types, \%relation_attributes, \@relation_modalities);
    $t3 = (times)[0];
...
#evaluate events
  if ((keys %{$ref_database{events}})>0 and (keys %{$tst_database{events}})>0) {
    ...
    score_releve_attribute_recognition ("event", \@event_types, \%event_attributes, \@event_modalities);
    $t3 = (times)[0];
...
#evaluate entity mentions
  if ((keys %{$ref_database{entities}})>0 and (keys %{$tst_database{entities}})>0) {
    undef %mapped_values, undef %mapped_document_values, undef %mention_map, undef %argument_map;
  ...
  foreach my $type ("entity", "quantity", "timex2", "relation", "event",
	...
	foreach my $mention (@{$ref->{documents}{$doc}{mentions}}) {
	  not defined $db->{refs}{$mention->{ID}} or die
	    ...
	    "for mentions of elements of type '$types' ".
...
#Timex2 parameters
  ...
  $timex2_mention_ref_fa_wgt = $p if defined ($p = $parms->{timex2_mention_ref_fa_wgt});
...
#Quantity parameters
  ...
  $quantity_mention_ref_fa_wgt = $p if defined ($p = $parms->{quantity_mention_ref_fa_wgt});
}
...
sub print_parameters {
  ...
  printf "\n Entity scoring is %s weighted\n".
    ...
    " Argument averaging is %s\n".
    " Scoring mode parameter set is \"$opt_P\"\n", uc $opt_W, uc $opt_M, uc $opt_N, uc $opt_S, uc $opt_A;
...
#Quantity parameters
  ...
  printf "  Value value (cost) weight for spurious (false alarm) values:%6.3f\n", $quantity_fa_wgt;
  printf "  Value mention value (cost) weight for spurious value mentions:%6.3f\n", $quantity_mention_fa_wgt;
  printf "  Value mention value (cost) discount for incorrect coreference:%6.3f\n", $quantity_mention_ref_fa_wgt;
}
...
#are discarded.  The database list of references is NOT updated.
  ...
  my $dbsrc = $db->{$srcname};
  my $dbdst = $db->{$dstname} = {};
  foreach my $key (keys %$dbsrc) {
    $dbdst->{$key} = copy_element ($dbsrc->{$key});
  }
}
...
sub score_quantity_detection {
  ...
  foreach my $doc (sort keys %$doc_costs) {
    my $ref_value = defined $doc_costs->{$doc}{REF} ? $doc_costs->{$doc}{REF} : 0;
    my $sys_value = $ref_value - defined $doc_costs->{$doc}{SYS} ? $doc_costs->{$doc}{SYS} : 0;
    printf "%10.2f %s for $doc\n", ($ref_value ? 100*$sys_value/$ref_value : $sys_value), "Value score";
  }
}
...
sub score_releve_attribute_recognition {
  ...
  my ($kind, $releve_types, $attributes, $modalities) = @_;

  my $attribute_stats = attribute_confusion_stats ($kind."s", [keys %$attributes]);
  ...
  print "\n", ucfirst "$kind Type confusion matrix for \"$source_type\" sources (for mapped ", $kind, "s):\n",
  ...
  # subtype attributes
  my $subtype_stats = subtype_confusion_stats ($kind."s");
  my ($subtype);
  foreach $ref_type (@$releve_types) {
    ...
    print "\n", ucfirst "$kind Subtype confusion matrix for \"$source_type\" sources (for mapped ", $kind, "s):\n";
  ...
  print "\n", ucfirst "$kind Modality confusion matrix for \"$source_type\" sources (for mapped ", $kind, "s):\n",
  ...
  my $role_stats = role_confusion_stats ($ref_database{$kind."s"});
...
#accumulate statistics over all documents
  ...
  while ((my $id, my $element) = each %{$ref_database{$elements}}) {
	...
	$cost = $doc_ref->{VALUE} eq $mapped_value ? 0 : $doc_ref->{VALUE} - $mapped_value;
...
#accumulate statistics over all documents
  my %role_stats;
  while ((my $id, my $element) = each %$db) {
    while ((my $doc, my $doc_ref) = each %{$element->{documents}}) {
      next unless my $doc_tst=$doc_ref->{MAP};
  ...
  while ((my $tst_role, my $tst_ids) = each %{$argument_map{$ref->{ID}}{$ref->{MAP}{ID}}}) {
    while ((my $tst_id, my $ref_arg) = each %$tst_ids) {
      $num_correctly_mapped++ if ($ref_database{refs}{$ref_arg->{ID}}{MAP} and
				  $ref_database{refs}{$ref_arg->{ID}}{MAP} eq $tst_database{refs}{$tst_id});
    }
  }
...
sub entity_mapping_details {
  ...
  foreach my $mention (sort compare_locators @data) {
    ...
    if ($mention->{MAP}) {
      next if $type eq "TST";
      my $ref_mention = $mention;
      my $tst_mention = $mention->{MAP};
      my $text = defined $ref_mention->{head}{text} ? $ref_mention->{head}{text} : "???";
      ...
      if ($error_type) {
	$print_data ||= $opt_e;
	$output .= sprintf (">>>   ref mention score: %.5f out of %.5f, \"%s\"",
			    $mention->{tst_scores}{$tst_mention}, $mention->{self_score}, $text);
	$output .= sprintf " (%3.3s/%3.3s/%3.3s) ID=$mention->{ID} -- MENTION $error_type MISMATCH (%3.3s/%3.3s/%3.3s)\n",
	...
	$output .= ">>>    tst mention extent = \"$text\" -- MENTION EXTENT MISMATCH\n";
      ...
      $print_data ||= $opt_e;
      $mention->{fa_score} = -$entity_fa_wgt*entity_mention_score ($mention, $mention)
	unless defined $mention->{fa_score};
      $output .= sprintf (">>>   %s mention score: %.5f out of %.5f, \"%s\"", lc $type,
			  ($type eq "TST" ? ($mention->{fa_score}, 0) : (0, $mention->{self_score})),
...
sub element_document_value {
  ...
  $ref = $tst if not $ref;
  $tst = $ref if not $tst;
  my $ref_mentions = (my $doc_ref = $ref->{documents}{$doc})->{mentions};
  my $tst_mentions = (my $doc_tst = $tst->{documents}{$doc})->{mentions};
  my $type = $ref->{ELEMENT_TYPE};
  my ($mention_fa_wgt, $mention_ref_fa_wgt, $fa_wgt) =
    ($type =~ /entity/ ? ($entity_mention_fa_wgt, $entity_mention_ref_fa_wgt, $entity_fa_wgt) :
     ($type =~ /quantity/ ? ($quantity_mention_fa_wgt, $quantity_mention_ref_fa_wgt, $quantity_fa_wgt) :
      ($type =~ /timex2/ ? ($timex2_mention_fa_wgt, $timex2_mention_ref_fa_wgt, $timex2_fa_wgt) :
  ...
  my @fa_scores;
  foreach my $mention (@$tst_mentions) { #compute FA scores
    if (not defined $mention->{fa_score}) {
      $mention->{self_score} = &$mention_scorer ($mention, $mention)
	unless defined $mention->{self_score};
      $mention->{fa_score} = -$mention->{self_score}*$mention_fa_wgt;
      $mention->{fa_score} *= $mention_ref_fa_wgt if $mention->{is_ref_mention};
    }
    push @fa_scores, $mention->{fa_score};
  }

  my ($mentions_score, %mapping_costs, $mentions_map);
  if ($ref eq $tst) { #compute self-score
    for (my $j=0; $j<@$tst_mentions; $j++) {
      $mentions_score += $compute_fa ? $fa_scores[$j] : $tst_mentions->[$j]{self_score};
      $mentions_map->{$j} = $j;
    }
  } else { #compute optimum mention mapping
    for (my $j=0; $j<@$tst_mentions; $j++) {
    ...
    for (my $j=0; $j<@$tst_mentions; $j++) {
      $mentions_score += $fa_scores[$j];
      next unless defined (my $i = $map->{$j});
      $mentions_map->{$i} = $j;
      $mentions_score -= $mapping_costs{$j}{$i};
    }
  }
  return undef unless defined $mentions_score;

  if ($type =~ /entity/ and $level_weighting) {
    my $weight = min ($entity_mention_type_wgt{$ref->{LEVEL}},
		      $entity_mention_type_wgt{$tst->{LEVEL}});
    my $max_score = $doc_ref->{mentions_score};
    if (not defined $max_score) {
      foreach my $mention (@$ref_mentions) {
	$mention->{self_score} = &$mention_scorer ($mention, $mention)
	  unless defined $mention->{self_score};
	$max_score += $mention->{self_score};
      }
      $doc_ref->{mentions_score} = $max_score;
    }
    $mentions_score = $weight * ($mentions_score/$max_score);
  }
...
# compute element value
  ...
  if ($type =~ /timex2/) {
    while ((my $attribute, my $weight) = each %timex2_attribute_wgt) {
      $value += $weight * ($ref->{$attribute} eq $tst->{$attribute} ? 1 : $epsilon)
	if ($ref->{$attribute} and $tst->{$attribute});
    }
    $value *= $fa_wgt if $compute_fa;
    $value = max($value,$epsilon);
  } else {
    $value = $type =~ /entity/ ?
      min($entity_type_wgt{$ref->{TYPE}}*$entity_class_wgt{$ref->{CLASS}},
	  $entity_type_wgt{$tst->{TYPE}}*$entity_class_wgt{$tst->{CLASS}}) :
	  min ($quantity_type_wgt{$ref->{TYPE}},
	       $quantity_type_wgt{$tst->{TYPE}});
    $value *= $fa_wgt if $compute_fa;
    $value = max($value,$epsilon);
    while ((my $attribute, my $weight) = each %{$type =~ /entity/ ? \%entity_err_wgt : \%quantity_err_wgt}) {
      $value *= $weight if (($ref->{$attribute} xor $tst->{$attribute}) or
			    ($ref->{$attribute} and $ref->{$attribute} ne $tst->{$attribute}));
    }
  }
  return $value*$mentions_score, $mentions_map;
}
...
sub releve_document_value {
  ...
  my $arg_db;
  $arg_db = $tst_database{refs} if not $ref;
  $arg_db = $ref_database{refs} if not $tst;
  $ref = $tst unless $ref;
  ...
  (my $type_wgt, my $modality_wgt, my $err_wgt, my $fa_wgt, my $role_err_wgt, my $arg_fa_wgt) =
    $relation_type ? (\%relation_type_wgt, \%relation_modality_wgt, \%relation_err_wgt,
		      $relation_fa_wgt, $relation_argument_role_err_wgt, $relation_argument_fa_wgt) :
		      (\%event_type_wgt, \%event_modality_wgt, \%event_err_wgt,
		       $event_fa_wgt, $event_argument_role_err_wgt, $event_argument_fa_wgt);
  if ($arg_db) { #compute self-score
    my $self_score = 0;
    while ((my $role, my $ids) = each %{$ref->{arguments}}) {
      foreach my $id (keys %$ids) {
	my $score = $arg_db->{$id}{documents}{$doc}{VALUE};
	$self_score += $geometric_averaging ? log $score : $score;
      }
    }
    $self_score = exp ($self_score/$nargs_ref) if $geometric_averaging and defined $self_score;
    $self_score += $epsilon * @$ref_mentions;
    my $value = $self_score*$type_wgt->{$ref->{TYPE}}*$modality_wgt->{$ref->{MODALITY}};
...
#compute argument score
  ...
  my $tst_score = my $cum_score = my $ref_score = my $nargs_mapped = 0;
  my $arg12_mismatch;
  while ((my $tst_role, my $tst_ids) = each %{$tst->{arguments}}) {
    foreach my $tst_id (keys %$tst_ids) {
      $tst_score += $tst_database{refs}{$tst_id}{documents}{$doc}{VALUE};
      next unless $arg_map and $arg_map->{$tst_role} and my $arg = $arg_map->{$tst_role}{$tst_id};
      (my $ref_id, my $ref_role) = ($arg->{ID}, $arg->{ROLE});
      my ($values, $map_score);
      $values = $mapped_document_values{$ref_id}{$tst_id} if $ref_id and $mapped_document_values{$ref_id};
      $map_score = $values->{$doc} if $values and defined $values->{$doc} and
	($allow_wrong_mapping or ($ref_database{refs}{$ref_id}{MAP} and
      ...
      $map_score = $ref_database{refs}{$ref_id}{documents}{$doc}{VALUE} if $mention_type and $map_score;
      my $score = $map_score;
  ...
  return undef if $geometric_averaging and $cum_score == 0;
  return undef unless $cum_score or $mentions_overlap;
  $cum_score = exp ($cum_score/$nargs_ref) if $geometric_averaging;
  $arg_fa_wgt = 0 if $mention_type;
  my $args_value = $cum_score - ($tst_score - $ref_score)*$arg_fa_wgt;
  $args_value += $mentions_overlap * $epsilon if $mentions_overlap;

#compute value
  my $value = min($type_wgt->{$ref->{TYPE}}*$modality_wgt->{$ref->{MODALITY}},
		  $type_wgt->{$tst->{TYPE}}*$modality_wgt->{$tst->{MODALITY}});

#reduce value for errors in attributes
  while ((my $attribute, my $weight) = each %$err_wgt) {
    my $mismatch = (($ref->{$attribute} xor $tst->{$attribute}) or
		    ($tst->{$attribute} and $ref->{$attribute} ne $tst->{$attribute}));
    $value *= $weight if ($mismatch or 
			  ($arg12_mismatch and not $relation_symmetry{$ref->{TYPE}} and
			   $attribute =~ /^(TYPE|SUBTYPE)$/));
  }
  return $value*$args_value, $mentions_map;
}
...
#compute argument mapping scores
  my %mapping_costs;
  (my $role_err_wgt, my $fa_wgt) = $ref->{ELEMENT_TYPE} =~ /relation/ ?
    ($relation_argument_role_err_wgt, $relation_argument_fa_wgt) :
    ($event_argument_role_err_wgt, $event_argument_fa_wgt);
  my $asymmetry_wgt = 1;
  if ($ref->{ELEMENT_TYPE} =~ /relation/ and not $relation_symmetry{$ref->{TYPE}}) {
    foreach my $attr ("TYPE", "SUBTYPE") {
      $asymmetry_wgt *= (1-$epsilon)*$relation_err_wgt{$attr} if $ref->{$attr} eq $tst->{$attr};
    }
  }
  for (my $i=0; $i<@ref_args; $i++) {
    ...
    for (my $j=0; $j<@tst_args; $j++) {
      ...
      my $values = $mapped_document_values{$ref_id}{$tst_id} if $mapped_document_values{$ref_id} and
	(($allow_wrong_mapping or ($ref_database{refs}{$ref_id}{MAP} and
				   $ref_database{refs}{$ref_id}{MAP} eq $tst_database{refs}{$tst_id}))
	 and ($ref->{ELEMENT_TYPE} =~ /event/ or
	      ($tst_args[$j]->{ROLE} eq $ref_args[$i]->{ROLE} or
	       ($asymmetry_wgt and $ref_args[$i]->{ROLE} =~ /^Arg-[12]$/ and $tst_args[$j]->{ROLE} =~ /^Arg-[12]$/)) or
	...
	$sys_score += $tst_database{refs}{$tst_id}{documents}{$doc}{VALUE};
      ...
      my $score = $map_score;
      $score *= $ref_args[$i]->{ROLE} =~ /^Arg-[12]$/ ? $asymmetry_wgt : $role_err_wgt
	if $ref_args[$i]->{ROLE} ne $tst_args[$j]->{ROLE};
      if ($geometric_averaging) {
	$score /= 0.001*$epsilon;
	$mapping_costs{$j}{$i} = -log $score;
      } else {
	$score += ($map_score-$sys_score)*$fa_wgt;
...
sub mapped_argument_description {
  ...
  (my $role_err_wgt, my $fa_wgt) = 
    (($ref and $ref->{ELEMENT_TYPE} =~ /relation/) or
  ...
  if ($src eq "ref" and $refarg and $tstarg) {
    ...
    my $cumscore = my $refscore = my $tstscore = 0;
    foreach my $doc (keys %docs) {
      if ($refarg and $tstarg) {
	$cumscore += $mapped_document_values{$refarg_id}{$tstarg_id}{$doc} if
	  $mapped_document_values{$refarg_id} and $mapped_document_values{$refarg_id}{$tstarg_id};
	$refscore += $ref_database{refs}{$refarg_id}{documents}{$doc}{VALUE};
	$tstscore += $tst_database{refs}{$tstarg_id}{documents}{$doc}{VALUE};
      } elsif ($refarg) {
	$refscore += $ref_database{refs}{$refarg_id}{documents}{$doc}{VALUE};
      } else {
	$tstscore += $tst_database{refs}{$tstarg_id}{documents}{$doc}{VALUE};
    ...
    my $wgtscore = $cumscore;
    $wgtscore *= $role_err_wgt if $refarg and $tstarg and $tstarg->{ROLE} ne $refarg->{ROLE};
    $out .= sprintf (" score: %.5f out of %.5f, ", $wgtscore - ($tstscore-$cumscore)*$fa_wgt, $refscore);
  }
...
sub releve_mention_mapping {
  ...
  foreach my $data (sort compare_locators @data) {
    ...
    $output .= $mention->{ELEMENT_TYPE} =~ /event mention/ ? 
      sprintf ("anchor=\"%s\", extent=\"%s\"", $mention->{anchor}{text}, $mention->{extent}{text}) :
      sprintf ("extent=\"%s\"", $mention->{extent}{text});
