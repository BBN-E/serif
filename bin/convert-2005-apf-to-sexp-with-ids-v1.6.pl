...
die "Usage key-file-dir key-file-suffix source-file-dir source-file-suffix output-file [NO-COREF]" unless @ARGV == 5 || @ARGV==6;

## Warn about unknown types only when this variable set to 1
my $VERIFY_TYPES = 0;

$no_coref = 0;
if (@ARGV==6) {
    $no_coref = 1;
    print "\n\n***REMOVING COREFERENCE INFORMATION***\n\n";
}
...
foreach $file ( @files ) {
    ...
    # use the file name if all else fails
    if ($docname eq "NOTFOUND") {
	$docname = $basename;
    }
...
#	}
	...
	if( $VERIFY_TYPES && !$entity_types{ $type } && !$event_types{ $type } ) {
	...
	for( $j = 0; $j < $entity_mention_list->getLength; $j++ ) {
	    ...
	    $str = "      ($mention_type $mention_role $start $end $head_start $head_end $mention_id)\n";
	    push @entity_mentions, $str;
	}
	# close individual entity
	if (!$no_coref) {
	    print OUT "    ($type $subtype SPC FALSE $entity_id\n";
	}
	foreach $mention (@entity_mentions) {
	    if ($no_coref) {
		## remove class info, we won't have it in the coref eval
		if ($mention =~ /\(PRONOUN/) {
		    print OUT "    (UNDET NONE SPC FALSE $docname-E$entity_id_counter\n";
		} else {
		    print OUT "    ($type $subtype SPC FALSE $docname-E$entity_id_counter\n";
		}
		$entity_id_counter++;
		print OUT $mention;
		print OUT "    )\n";
	...
	if( $VERIFY_TYPES && !$value_types{ $type } ) {
	...
	for( $j = 0; $j < $relation_mention_list->getLength; $j++ ) {
	    ...
	    $rel_mention_id =~ tr/ /_/;
	    $rel_arg_list = $relation_mention->getElementsByTagName( "relation_mention_argument" );
	    my $arg1;
	    my $arg2;
	    my $time_arg = "NULL";
	    my $time_role = "NULL";
		...
		$arg_id =~ tr/ /_/;
		$argnum = $arg->getAttribute( "ROLE" );
		if ($argnum eq "Arg-1") {
		    $arg1 = $arg_id;
		} elsif ($argnum eq "Arg-2") {
		    $arg2 = $arg_id;
		} else {
		    $time_arg = $arg_id;
		    $time_role = $argnum;
